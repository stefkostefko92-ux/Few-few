...
export const GET = gestito(async () => {
  ...
  const [
    ...
    prisma.impianto.count({ where: { attivo: true } }),
    prisma.impianto.groupBy({ by: ["stato"], _count: true, where: { attivo: true } }),
    prisma.ordineLavoro.count({
      where: { stato: { notIn: ["CHIUSO", "ANNULLATO"] } },
    }),
    prisma.ordineLavoro.groupBy({ by: ["stato"], _count: true }),
    prisma.ordineLavoro.groupBy({
      by: ["priorita"],
      _count: true,
      where: { stato: { notIn: ["CHIUSO", "ANNULLATO"] } },
    }),
    prisma.preventivo.groupBy({ by: ["stato"], _count: true }),
    prisma.preventivo.count({ where: { stato: "INVIATO" } }),
    prisma.scadenzaImpianto.findMany({
      where: { completata: false, dataScadenza: { lte: fra30 } },
    ...
    prisma.$queryRaw<
      { id: string; codice: string; nome: string; quantita: number; sogliaMinima: number }[]
    >`SELECT id, codice, nome, quantita, "sogliaMinima"
      FROM articoli_magazzino
      WHERE attivo = true AND quantita < "sogliaMinima"
  ...
  const automezziPerStato = { verde: 0, giallo: 0, rosso: 0 } as Record<string, number>;
  for (const a of automezzi) automezziPerStato[a.stato] = (automezziPerStato[a.stato] ?? 0) + 1;
  ...
  const base = {
    ...
    impiantiPerStato: impiantiPerStato.map((r) => ({ nome: r.stato, valore: r._count })),
    ordiniPerStato: ordiniPerStato.map((r) => ({ nome: r.stato, valore: r._count })),
    ordiniPerPriorita: ordiniPerPriorita.map((r) => ({ nome: r.priorita, valore: r._count })),
    preventiviPerStato: preventiviPerStato.map((r) => ({ nome: r.stato, valore: r._count })),
    automezziPerStato: Object.entries(automezziPerStato).map(([nome, valore]) => ({
      nome,
      valore,
    })),
    scadenzeProssime,
  ...
  const [fatture, insolute] = await Promise.all([
    prisma.fattura.findMany({
      where: { tipo: "EMESSA", stato: { not: "STORNATA" }, data: { gte: inizio12Mesi } },
      select: { data: true, totaleLordo: true, stato: true },
    }),
    prisma.fattura.findMany({
      where: { tipo: "EMESSA", stato: "SCADUTA" },
      select: { totaleLordo: true },
    }),
  ]);

  const mesi = mesiIndietro(12);
  const perMese = new Map(mesi.map((m) => [m.chiave, { emesso: 0, incassato: 0 }]));
  ...
  const totaleInsoluto = insolute.reduce((acc, f) => acc + toCents(f.totaleLordo.toString()), 0);

  return ok({
    ...base,
    fatturatoMensile,
    insoluti: { numero: insolute.length, totale: Number(fromCents(totaleInsoluto)) },
  });
});
