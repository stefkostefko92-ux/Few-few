...
import { sogliePendenti, giorniRimanenti, statoAutomezzo } from "../scadenze-logic";

const OGGI = new Date("2026-07-25T00:00:00Z");
function fra(n: number): Date {
  return new Date(OGGI.getTime() + n * 86_400_000);
}
const base = { notificato90: false, notificato60: false, notificato30: false, completata: false };

test("на 100 дни — нищо; на 80 — само 90", () => {
  assert.deepEqual(sogliePendenti({ ...base, dataScadenza: fra(100) }, OGGI), []);
  assert.deepEqual(sogliePendenti({ ...base, dataScadenza: fra(80) }, OGGI), [90]);
});

test("на 25 дни без изпратени — и трите наведнъж", () => {
  assert.deepEqual(sogliePendenti({ ...base, dataScadenza: fra(25) }, OGGI), [90, 60, 30]);
});

test("вече вдигнат флаг не се повтаря", () => {
  assert.deepEqual(
    sogliePendenti({ ...base, notificato90: true, notificato60: true, dataScadenza: fra(25) }, OGGI),
    [30]
  );
});
