...
// ГРАНИЦАТА НА ЗАКРЪГЛЯНЕТО, ВЗЕТА ОТДЕЛНО. Half-up се проверяваше досега само
// през `totaleVoce`/`ivaVoce`, които закръглят със СОБСТВЕНА аритметика. Самата
// `toCents` се вика директно (redditivita, totali-db, sdi/fatturapa, pdf/carica)
// върху ИЗЧИСЛЕНИ стойности, които Zod-схемата `dec` никога не е видяла — тоест
// с три и повече десетични. Точката „ровно .xx5" е единственото място, където
// half-up се различава от отрязване, и остава невидима за покритието: редът се
// изпълнява, но границата не се пробва.
test("toCents: half-up точно на границата .xx5", () => {
  assert.equal(toCents("1.234"), 123); // под границата → надолу
  assert.equal(toCents("1.235"), 124); // РОВНО на границата → нагоре
  assert.equal(toCents("1.236"), 124);
  assert.equal(toCents("0.005"), 1); // цялата стойност е под цент
  assert.equal(toCents("0.004"), 0);
  // Away from zero: -1.235 е -1.24, не -1.23. Обратното би дало на клиента
  // цент в повече при всяко кредитно известие.
  assert.equal(toCents("-1.235"), -124);
  assert.equal(toCents("-1.234"), -123);
  // Повече от три десетични: решава ТРЕТАТА, останалите не носят половинка.
  assert.equal(toCents("1.2349999"), 123);
  assert.equal(toCents("1.2350001"), 124);
});
