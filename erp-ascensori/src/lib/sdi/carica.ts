...
export async function fatturaPerSdi(
  ...
  return {
    ...
    // б. „а" D.P.R. 633/1972 → TD24; иначе TD01.
    //
    // Сторнирането НЕ сменя вида на ТОЗИ файл. Преди STORNATA даваше TD04 със
    // СЪЩИЯ номер и име на файл: SdI го отхвърля като дубликат, а пратката за
    // conservazione губеше издадения TD01. Кредитното известие (TD04) е ОТДЕЛЕН
    // документ със свой номер (чл. 26 D.P.R. 633/1972) — издава се през
    // посредника, докато гестионалът не го прави сам.
    tipoDocumento: f.ddt.length > 0 ? "TD24" : "TD01",
