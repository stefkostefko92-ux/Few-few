...
- **Автоматизми**: `scadenze-runner.ts` — прагове 90/60/30 (флагове еднократно),
  ...
  напускат одита: 6 месеца за LOGIN/LOGOUT (Provv. Garante), 10 години за
  останалото (чл. 2220 c.c.), 90 дни за телеметрията; непознато действие получава
  ДЪЛГИЯ срок. И двата пишат следа в `automatismi_run` (dead-man проверката я чете).
- **Изолацията по фирма важи и ИЗВЪН CRUD фабриката.** Всеки маршрут със своя
  логика (смяна на статус, редове по документ, движения, потребители) чете с
  `findFirst` + `filtroTenant(s)`, не с `findUnique` — иначе познат UUID отваря
  чужд запис. `filtroUtenti(s)` е изключението: MASTER е ниво на доставчика и
  вижда всички фирми.
