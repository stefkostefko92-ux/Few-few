...
|-------|------|-----------|
| `LOGIN` / `LOGOUT` | 6 месеца | Provv. Garante 27/11/2008 (amministratori di sistema), потвърдено с 467/2018 |
| Останалият одит | 10 години | чл. 2220 Codice Civile, чл. 22 D.P.R. 600/1973 |
| Пускания на автоматизми | 90 дни | оперативна телеметрия, не лични данни |

Действие, което кодът не познава, попада в **дългия** срок — грешката в тази посока
е „пазим твърде дълго“, не „изтрихме доказателство“. Логиката е чиста и тествана
(`src/lib/retention-logic.ts`). Това не е правен съвет: сроковете се сверяват с
адвоката на клиента преди продукция.
...
3. **Верижен подпис на одита** (`hmacPrecedente`) — сегашната схема лови промяна на
   ред, но не и изтриване на цял ред.
4. **Row-Level Security** в Postgres като втори слой на мулти-тенант изолацията.
