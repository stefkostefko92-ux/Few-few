...
describe("ключове за публичното API", () => {
  test("ключът се показва ВЕДНЪЖ и после само с префикс", async () => {
    ...
    const lista = await admin.get<{ righe: Record<string, unknown>[] }>(
      "/api/chiavi",
    );
    ...
    for (const r of lista.dati.righe) {
      ...
      assert.equal(
        JSON.stringify(r).includes(chiave),
        false,
        "ключът се вижда втори път",
      );
    }
  });

  test("ключ без права не се създава", async () => {
    const r = await admin.post("/api/chiavi", {
      etichetta: "Vuota",
      ambiti: [],
    });
...
describe("достъп с ключ", () => {
  ...
  test("без ключ и с непознат ключ — 401", async () => {
    ...
    assert.equal(
      (await conChiave("/api/pubblica/v1/impianti", "ea_live_inesistente"))
        .status,
      401,
    );
  ...
  test("отмененият ключ спира да работи веднага", async () => {
    const creata = await admin.post<{ id: string; chiave: string }>(
      "/api/chiavi",
      {
    ...
    assert.equal(
      (await conChiave("/api/pubblica/v1/impianti", creata.dati.chiave)).status,
      200,
    );
  ...
  test("ключът вижда САМО своята фирма", async () => {
    ...
    assert.equal(r.status, 200);
    assert.equal(
      (r.dati as { totale: number }).totale,
      0,
      "ключът вижда чужди импианти",
    );
  });
});
...
describe("webhooks", () => {
  test("тайната се показва веднъж и не излиза в списъка", async () => {
    const r = await admin.post<{ id: string; segreto: string }>(
      "/api/webhooks",
      {
  ...
  test("смяната на статус поражда доставка", async () => {
    ...
    assert.equal(
      (
        await direzione.richiesta("PATCH", `/api/fatture/${f.dati.id}/stato`, {
          stato: "EMESSA",
        })
      ).status,
      200,
    ...
    const lista = await admin.get<{
      righe: { id: string; _count: { consegne: number } }[];
    }>("/api/webhooks");
