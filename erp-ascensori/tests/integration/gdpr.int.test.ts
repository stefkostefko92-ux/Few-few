...
describe("права на субекта (GDPR)", () => {
  ...
  test("износът е машинно четим и БЕЗ удостоверения", async () => {
    ...
    assert.match(
      ...
      /attachment; filename="gdpr-utente-/,
    );
    ...
    assert.equal(
      "password" in dati.soggetto,
      false,
      "хешът на паролата излиза навън",
    );
    assert.equal(
      "totpSegreto" in dati.soggetto,
      false,
      "тайната на втория фактор излиза навън",
    );
  ...
  test("анонимизацията иска ИЗРИЧНО потвърждение", async () => {
    const u = await nuovoUtente();
    assert.equal(
      (await admin.post(`/api/gdpr/utente/${u.id}/anonimizza`, {})).status,
      400,
    );
    assert.equal(
      (
        await admin.post(`/api/gdpr/utente/${u.id}/anonimizza`, {
          conferma: false,
        })
      ).status,
      400,
  ...
  test("след анонимизация лицето го няма, а следата остава", async () => {
    ...
    assert.equal(
      (await u.sessione.richiesta("POST", "/api/auth/refresh")).status,
      401,
    );

    const dopo = await scarica(admin, `/api/gdpr/utente/${u.id}/esporta`);
    const dati = JSON.parse(dopo.testo) as {
      soggetto: Record<string, unknown>;
    };
    ...
    const v = await admin.post<{ integro: boolean }>("/api/audit/verifica", {
      limite: 300,
    });
    ...
    assert.equal(
      v.dati.integro,
      true,
      "анонимизацията счупи веригата на одита",
    );
  });
  ...
  test("самата операция влиза в регистъра", async () => {
    ...
    const reg = await admin.get<{
      righe: { entita: string; entitaId: string }[];
  ...
  test("под нивото на ADMIN правата не се упражняват", async () => {
    ...
    assert.equal(
      (await operatore.get(`/api/gdpr/utente/${u.id}/esporta`)).status,
      403,
    );
    assert.equal(
      (
        await operatore.post(`/api/gdpr/utente/${u.id}/anonimizza`, {
          conferma: true,
        })
      ).status,
      403,
  ...
  test("чуждо лице не се изнася с познат идентификатор", async () => {
    ...
    assert.equal(
      (await altra.get(`/api/gdpr/utente/${u.id}/esporta`)).status,
      404,
    );
    assert.equal(
      (
        await altra.post(`/api/gdpr/utente/${u.id}/anonimizza`, {
          conferma: true,
        })
      ).status,
      404,
