...
describe("регистър на операциите", () => {
  test("операциите се записват и подписът се проверява като валиден", async () => {
    ...
    const reg = await admin.get<{ righe: { entita: string; entitaId: string; azione: string }[] }>(
      "/api/audit?azione=CREATE&size=50"
    );
    assert.equal(reg.status, 200);
    assert.ok(
      reg.dati.righe.some((r) => r.entitaId === creato.dati.id && r.entita === "condomini"),
      "създаването трябва да е в регистъра"
    );

    const ver = await admin.post<{ integro: boolean; corrotte: string[] }>("/api/audit/verifica", {
      limite: 200,
    });
    assert.equal(ver.status, 200);
    assert.equal(ver.dati.integro, true, `невалидни подписи: ${ver.dati.corrotte?.length}`);
  });

  test("регистърът няма маршрут за промяна или изтриване", async () => {
    const reg = await admin.get<{ righe: { id: string }[] }>("/api/audit?size=1");
    const id = reg.dati.righe[0]?.id;
    assert.ok(id, "трябва да има поне един запис");

    for (const metodo of ["PUT", "PATCH", "DELETE"] as const) {
      const { status } = await admin.richiesta(metodo, `/api/audit/${id}`, { azione: "X" });
      assert.ok(
  ...
  test("входът и изходът също се записват", async () => {
    const reg = await admin.get<{ righe: { azione: string }[] }>("/api/audit?azione=LOGIN&size=5");
...
describe("преходи на състоянието при документите", () => {
  test("фактура: не се прескача от PAGATA обратно в BOZZA", async () => {
    ...
    const indietro = await master.patch(`/api/fatture/${id}/stato`, { stato: "BOZZA" });
  ...
  test("оферта: отхвърлената не се съживява в чернова", async () => {
    const p = await master.post<{ id: string }>("/api/preventivi", { oggetto: unico("Prev") });
    const id = p.dati.id;
    await master.patch(`/api/preventivi/${id}/stato`, { stato: "INVIATO" });
    await master.patch(`/api/preventivi/${id}/stato`, { stato: "RIFIUTATO" });

    const indietro = await master.patch(`/api/preventivi/${id}/stato`, { stato: "BOZZA" });
...
describe("детайлът на импианта не филтрира на клиента", () => {
  test("свързаните записи се четат филтрирани от сървъра", async () => {
    ...
    const filtrate = await master.get<{ righe: { impiantoId: string }[]; totale: number }>(
      `/api/scadenze?impiantoId=${id}`
    );
    assert.equal(filtrate.status, 200);
    assert.ok(filtrate.dati.righe.length > 0, "трябва да върне срока на този импиант");
    assert.ok(
