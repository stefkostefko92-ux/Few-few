...
import { consegnaInAttesa } from "../src/lib/webhook/emetti";
import { prisma } from "../src/lib/prisma";

async function main() {
  const esito = await consegnaInAttesa(Number(process.env.WEBHOOK_LOTTO ?? 100));
  console.log(
    `▸ consegne: tentate ${esito.tentate} · riuscite ${esito.riuscite} · fallite ${esito.fallite}` +
      (esito.webhookDisattivati ? ` · webhook disattivati ${esito.webhookDisattivati}` : ""),
  );
  await prisma.$disconnect();
}

main().catch(async (e) => {
  console.error("✗ доставката се провали:", e instanceof Error ? e.constructor.name : typeof e);
  await prisma.$disconnect();
  process.exit(1);
});
