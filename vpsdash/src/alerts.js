...
export class AlertEngine {
  ...
  // Паметта за събитията изтича. Прагът е щедър (10 cooldown-а, минимум едно
  // денонощие), защото задачата ѝ е да ПОТИСКА повторение, а не да помни история;
  // за история има одит. Таванът е втори предпазител: ключ, който се сменя при
  // всяко събитие, не бива да може да надуе паметта дори в рамките на прага.
  pruneTransient(now = Date.now(), cooldownMs = 0) {
    const m = this.transientSeen;
    if (!m || !m.size) return;
    const maxAge = Math.max(24 * 3600 * 1000, cooldownMs * 10);
    for (const [key, seen] of [...m]) {
      if (now - seen > maxAge) m.delete(key);
    }
    const CAP = 500;
    if (m.size > CAP) {
      // Най-старите падат първи — те са и най-безполезни за потискането.
      const oldest = [...m].sort((a, b) => a[1] - b[1]).slice(0, m.size - CAP);
      for (const [key] of oldest) m.delete(key);
    }
  }
  ...
  async evaluate() {
    ...
    // Чистене на паметта за СЪБИТИЯТА. `transientSeen` пази „кога го видях за
    // последно", за да не се повтаря известието — но ключът на някои събития носи
    // ВРЕМЕТО в себе си (`ssh:login:<хеш>:<ts>` е нов при всеки вход). Без изтичане
    // картата расте, докато панелът върви: ~3600 записа на година при десет
    // влизания на ден, а при брутфорс с успели опити — десетки хиляди. Запис,
    // по-стар от няколко cooldown-а, вече не влияе на НИТО едно решение; държим го
    // само защото никой не го е махнал. Точно този клас теч не се забелязва — той
    // не чупи нищо, просто расте.
    this.pruneTransient(now, cooldownMs);
