...
const DEFAULTS = {
  ...
  alerts: {
    ...
    notifyInfo: false,
    // Седмичен дайджест: „жив съм и ето какво видях" — тишината на алармите не
    // доказва здраве. Ден 0-6 (нд-сб) и час, по часовника на сървъра.
    digest: { enabled: true, weekday: 1, hour: 8 },
  ...
  backups: {
    ...
    // Снимката се ПРАВИ сама. Часът е фиксиран (нощем), а не „на всеки 24 часа от
    // последния път" — иначе бекъпът пълзи през деня и някой ден пада върху пиков
    // час или върху деплой. Първото пускане чака нощния час нарочно.
    schedule: { enabled: true, everyHours: 24, atHour: 3 },
    // 3-2-1: копие на другия VPS през federation. Изключено по подразбиране, защото
    // иска peer с TLS и място там — включването е решение, не подразбиране.
    offsite: { enabled: false, intervalSec: 3600, perRun: 3, maxMB: 4096, keep: 10 },
  },
  // Тихите пазачи: дрейф на /etc отпечатъка (каданс, сек) + нови SSH входове.
  guardians: { enabled: true, etcCheck: { intervalSec: 1800 } },
  // Месечен трафик срещу квотата на хостера. Без `quotaTB` панелът само ПОКАЗВА
  // (няма с какво да сравни). Мери се изходящият — почти всички хостери таксуват
  // само egress; сборът от двете посоки би алармирал за трафик, който не се плаща.
  traffic: { enabled: true, quotaTB: null, countDirection: 'tx', ifaces: [] },
  ...
  acmeEmail: '',
  // Портове: алармата е за НОВО изложен порт спрямо приета базова линия, а не
  // за „443 е отворен". Базовата линия се приема от секция „Портове" и живее в
  // state, не тук.
  ports: { enabled: true },
  // Незадължителен графичен десктоп (Ubuntu + XFCE в контейнер). ИЗКЛЮЧЕН по
  // подразбиране и не стартира с машината — пуска се от панела при нужда.
  // Слуша само на 127.0.0.1 и се вижда единствено през панела, тоест зад
  // неговия вход и 2FA. `composeFile` празно = търси се в текущия release.
  desktop: { port: 3010, composeFile: '' },
