...
export const SUDO_ON_WRITE = [
  // Смяната на парола: открадната сесия иначе сменя ключа на собственика и го
  // заключва отвън — точно обратното на това, за което съществува маршрутът.
  /^\/api\/auth\/password$/,
  /^\/api\/power$/,
  // Чистенето на ядра ТРИЕ пакети — сгрешеното ядро прави машината незагрузима,
  // а това е поправка, от която няма връщане през панела (той също няма да се
  // вдигне). Класификацията пази текущото и най-новото, но решението остава на
  // човек с парола.
  /^\/api\/updates\/kernel-clean$/,
  // Освобождаването ТРИЕ. Категориите са подбрани да не съдържат данни, но
  // необратимото си остава необратимо — парола, преди да изчезне нещо.
  /^\/api\/reclaim\/run$/,
  ...
  // Свиването на журнала ТРИЕ логове — точно това прави човек, който заличава
  // следи. Мястото, което освобождава, не си струва да е на един клик от
  // открадната сесия.
  /^\/api\/disk\/vacuum$/,
  // Ключът за шифрирания бекъп на панела отключва архив, който носи ВСИЧКИ тайни
  // на панела — показването му е разкриване на тайна от класа на `.env` reveal.
