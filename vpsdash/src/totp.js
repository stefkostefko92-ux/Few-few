...
// Приемане на стъпка — ЕДНО място за правилото „код не се ползва два пъти".
//
// Дотук правилото живееше на две места (вход и sudo) и в двата беше проверка за
// РАЗЛИЧИЕ (`step !== last`). Това не е достатъчно: прозорецът приема три
// стъпки едновременно (±1), значи след успешен вход с кода за стъпка S, код от
// стъпка S−1 — заснет тридесет секунди по-рано — още минава, защото S−1 ≠ S.
// RFC 6238 §5.2 иска СТРОГО НАРАСТВАНЕ: приема се само стъпка, по-нова от
// последната приета. Тогава заснет код става безполезен в мига, в който
// собственикът влезе.
//
// Изнесено в обща функция, защото двата екземпляра вече бяха се разминали в
// детайли — дублирана проверка за сигурност дрейфва по подразбиране.
export function acceptStep(state, step) {
