...
test('десктопът слуша САМО на loopback и не се вдига сам', () => {
  ...
  // Портът МОЖЕ да е заместване (`desktop.port` от конфига го мени, защото 3010
  // не е свободен навсякъде) — АДРЕСЪТ не може и никога няма да може. Променлива
  // на негово място би позволила на конфиг ред да премести десктопа на 0.0.0.0,
  // тоест да го изкара публичен покрай входа на панела и 2FA-та. Затова тук
  // `127.0.0.1:` е литерал в самия израз, а не нещо, което се разрешава наум.
  assert.match(
    yml,
    /'127\.0\.0\.1:(?:\d+|\$\{DESKTOP_PORT:-\d+\}):\d+'/,
    'портът трябва да е вързан за 127.0.0.1 (само НОМЕРЪТ му може да е променлива)'
  );
  assert.ok(
    !/-\s*'?\$\{[^}]*\}:\d+:\d+/.test(yml),
    'адресът за слушане НЕ бива да идва от променлива — това е пътят към публичен десктоп'
  );
