...
`npm install` е нужен САМО за конвейера на снимките (`sharp`, devDependency). **Визията на бранд страниците е
тази на „Двубой в Рейвънхолд“** (`boy/template.html`, клон `claude/medieval-3d-fight-animation-9ybhpo`), по
skill-а frontend-design (2026-09-25): виж „Визия“ по-долу. По нареждане на собственика за бранд сайта: БЕЗ
prefers-reduced-motion гейт в хъба (демотата го уважават); без строб._
...
## Визия (бранд страниците — хъб, цени, проекти, вертикали, блог, правна)

- **Токени** (`src/assets/site.css` `:root`, взети от `boy/template.html`): мастило `#07070a` (фон) · пергамент
  `#ebe2ce` (заглавия) / `#c2b9a7` (текст) / `#948c7d` (приглушено) · **жарава `#ff8a3d` = действие** (основният
  бутон, линкове в текст) · **злато `#c9a24a` = структура и цени** (етикети, черти, цени) · цианът `0,229,255` е
  САМО ядрото в бурята (hero.js / CSS постера). Старите имена (`--cyan`, `--off`, `--text`…) са псевдоними за
  admin.css и inline стилове — не ги ползвай в нов код.
- **Шрифтове** (`node tools/fonts.mjs --brand` → `src/assets/fonts/brand.css`): Alegreya 500–800 (заглавия) ·
  Alegreya Sans 400 (текст) · Alegreya Sans SC 500 (етикети — истински малки главни, не `text-transform`). Grenze
  Gotisch на boy няма кирилица. Цифрите в заглавия/цени са `lining-nums` (Alegreya по подразбиране е oldstyle).
  Inter Tight живее отделно в `inter-tight.css` — САМО кирилският резерв на демотата.
- **Смелостта е на едно място** — бурята в hero-то. Всичко друго е тихо: без кутии (клетка = текст под златна
  черта), без главни букви и `// ТАГ`, без моно етикети, без стрелки в бутоните, без анимации по секциите; единият
  оркестриран момент е появата на надписа в hero-то. **Номера само за последователност** (процесът → римски глави
  `.chapters`). Демотата са `.reel` (3 колони, средната свалена); цените в хъба са ценоразпис с точки (`.mini`).
- **Съзнателно махнато** с новата визия (тестът пази да не се върнат): boot екран, курсор, глич при клик,
  scramble, магнитни бутони/букви, ехо заглавия (`ghost()` вече връща обикновен `h2.h2`), тикер, FPS HUD, reveal.
  i18n низовете са в изречение (без главни и стрелки) — CSS не ги трансформира.
...
- **Производителност (гейтната в теста)**: един общ `requestAnimationFrame` цикъл в `site.js` (`FRAME[]`, днес
  само FPS детекторът); **без `backdrop-filter` на фиксирани/sticky навигации** (преизчислява се на всеки scroll
  кадър); `text-wrap:balance` само на h1/h2 (на десетки h3 + подмяна на шрифта = дълги задачи след FCP); бранд
  шрифтовете са 3 файла на език (всеки нов тегловен вариант е още една прекомпоновка — Sans 500 го махнахме: TBT
  166 → 19 ms).
  **Hero = езикът на „Двубой в Рейвънхолд“ (`boy/`, клон `claude/medieval-3d-fight-animation-9ybhpo`)**:
  ...
  — така a11y проверката вижда истинския фон. **LITE режим** (`html.lite`): по Navigator API (≤4 ядра / ≤4 GB /
