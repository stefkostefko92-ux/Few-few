...
function field(id, label, value, kind = "input") {
  const v = esc(value);
  return `<label class="cms-field"><span>${esc(label)}</span>${kind === "textarea" ? `<textarea data-bind="${id}" rows="3">${v}</textarea>` : `<input data-bind="${id}" value="${v}">`}</label>`;
}
...
export function renderAdmin(lang) {
  ...
  const panes = {
    ...
    services: `<h2>${esc(a.nav.services)}</h2>${services.map((s, i) => `<div class="cms-row"><label class="cms-field grow"><span>${esc(a.fields.name)}</span><input data-bind="s${i}n" value="${esc(s.t)}"></label><label class="cms-field"><span>${esc(a.fields.price)}</span><input data-bind="s${i}p" value="${esc(s.p || "")}"></label></div>`).join("")}`,
    photos: `<h2>${esc(a.nav.photos)}</h2><p class="cms-hint">${esc(a.fields.drop)}</p><div class="cms-photos">${["hero", "g1", "g2", "g3"].map((s) => `<figure class="cms-photo"><img data-photo="${s}" src="${img(s)}" alt="" width="450" height="300" loading="lazy"><label class="cms-replace">${esc(a.fields.replace)}<input type="file" accept="image/*" data-file="${s}" hidden></label></figure>`).join("")}</div>`,
    reviews: `<h2>${esc(a.nav.reviews)}</h2>${t.reviews.map((r, i) => `<div class="cms-review"><label class="cms-field grow"><span>${esc(a.fields.name)}</span><input data-bind="r${i}n" value="${esc(r.name)}"></label><label class="cms-field grow"><span>${esc(a.fields.text)}</span><textarea data-bind="r${i}t" rows="2">${esc(r.text)}</textarea></label><label class="cms-switch"><input type="checkbox" data-bind="r${i}on" checked><span>${esc(a.fields.published)}</span></label></div>`).join("")}`,
    settings: `<h2>${esc(a.nav.settings)}</h2>${field("phone", a.fields.phone, t.phone)}<div class="cms-field"><span>${esc(a.fields.langs)}</span><div class="cms-langs">${LANGS.map((l) => `<label class="cms-switch"><input type="checkbox" data-bind="lang-${l}" checked><span>${I18N[l].short}</span></label>`).join("")}</div></div>`,
  };
  const preview = `<div class="cms-phone"><div class="cms-screen"><div class="cms-pv-nav"><b>${esc(t.name)}</b><span data-pv="phone">${esc(t.phone)}</span></div><div class="cms-pv-hero"><img data-pv-photo="hero" src="${img("hero")}" alt="" width="450" height="300"><div><small data-pv="eyebrow">${esc(t.hero.eyebrow)}</small><h3 data-pv="title">${esc(t.hero.title.replace(/<[^>]+>/g, ""))}</h3><p data-pv="lede">${esc(t.hero.lede)}</p><span class="cms-pv-btn" data-pv="cta">${esc(t.hero.cta)}</span></div></div><ul class="cms-pv-services">${services.map((s, i) => `<li><span data-pv="s${i}n">${esc(s.t)}</span><b data-pv="s${i}p">${esc(s.p || "")}</b></li>`).join("")}</ul><div class="cms-pv-gallery">${["g1", "g2", "g3"].map((s) => `<img data-pv-photo="${s}" src="${img(s)}" alt="" width="450" height="300" loading="lazy">`).join("")}</div><div class="cms-pv-reviews">${t.reviews.map((r, i) => `<blockquote data-pv-review="${i}"><p data-pv="r${i}t">${esc(r.text)}</p><footer data-pv="r${i}n">${esc(r.name)}</footer></blockquote>`).join("")}</div><div class="cms-pv-langs">${LANGS.map((l) => `<span data-pv-lang="${l}">${I18N[l].short}</span>`).join("")}</div></div></div>`;
