...
import { siteNav, siteFooter, contact, ghost, HUB_FONTS, BRAND_BG } from "./hub.mjs";
...
function market(p, lang) {
  ...
  return `<section class="section alt" id="market"><div class="wrap"><div class="tag">2026</div>${ghost(esc(p.marketTitle))}<p class="lede">${esc(p.marketLede)}</p><div class="table-wrap"><table class="table"><thead><tr><th>${esc(p.marketCols.item)}</th><th>${esc(p.marketCols.bg)}</th><th>${esc(p.marketCols.it)}</th><th>${esc(p.marketCols.eu)}</th></tr></thead><tbody>${MARKET_RANGES.map((r) => { const s = monthly.has(r.id) ? " " + p.perMonth : ""; return `<tr><th scope="row">${esc(p.marketRows[r.id])}</th><td>${range(r.bg, lang, s)}</td><td>${range(r.it, lang, s)}</td><td>${range(r.eu, lang, s)}</td></tr>`; }).join("")}</tbody></table></div></div></section>`;
}

function tiers(p, lang) {
  return `<section class="section" id="packages"><div class="wrap">${ghost(esc(p.packagesTitle))}<p class="lede">${esc(p.packagesLede)}</p><div class="tiers">${TIERS.map((t) => { const x = p.tiers[t.id]; return `<article class="tier${t.popular ? " pop" : ""}" id="${t.id}">${t.popular ? `<span class="pop-badge">${esc(p.popular)}</span>` : ""}<p class="tier-tag">${esc(x.tag)}</p><h3>${esc(x.name)}</h3><p class="tier-desc">${esc(x.desc)}</p><p class="tier-price"><strong>${money(shown(t.price, lang), lang)}</strong><span class="tier-market"><s>${money(shownMarket(t.market, lang), lang)}</s> ${esc(p.market)} · <b>−${discountPct(t)}% ${esc(p.saving)}</b></span></p><ul>${x.features.map((f) => `<li>${ICON.check}<span>${esc(f)}</span></li>`).join("")}</ul><p class="tier-delivery">${ICON.clock} ${esc(p.delivery)}: ${t.days[0]}–${t.days[1]} ${esc(p.days)}</p><a class="btn ${t.popular ? "btn-solid" : ""}" href="mailto:${BRAND_EMAIL}?subject=${encodeURIComponent(x.name + " — Carbon Stealth")}">${esc(p.choose)}</a></article>`; }).join("")}</div></div></section>`;
}

function addons(p, lang) {
  return `<section class="section alt" id="addons"><div class="wrap">${ghost(esc(p.addonsTitle))}<p class="lede">${esc(p.addonsLede)}</p><div class="addons">${ADDONS.map((a) => `<div class="addon"><span>${esc(p.addons[a.id])}</span><span class="addon-price"><strong>${money(shown(a.price, lang), lang)}</strong> <small>${esc(a.kind === "monthly" ? p.monthly : p.once)}</small><em>−${discountPct(a)}%</em></span></div>`).join("")}</div></div></section>`;
}

function vat(p) {
  const v = p.vat;
  return `<section class="section" id="vat"><div class="wrap">${ghost(esc(p.vatTitle))}<div class="grid1" style="margin-top:40px">${v.items.map(([h, d], i) => `<article class="cell${i === 1 ? " vat-eu" : ""}"><span class="ic">${i === 0 ? ICON.euro : i === 1 ? ICON.globe : ICON.layers}</span><h3>${esc(h)}</h3><p>${esc(d)}</p></article>`).join("")}</div><p class="tiny vat-note">${esc(v.note)}</p></div></section>`;
}

function compare(p, lang) {
  return `<section class="section" id="compare"><div class="wrap">${ghost(esc(p.compareTitle))}<p class="lede">${esc(p.compareLede)}</p><div class="table-wrap"><table class="table table-compare"><thead><tr><th></th>${p.compareCols.map((c, i) => `<th${i === 0 ? ' class="us"' : ""}>${esc(c)}</th>`).join("")}</tr></thead><tbody>${p.compareRows.map((r) => `<tr><th>${esc(r[0])}</th>${r.slice(1).map((c, i) => `<td${i === 0 ? ' class="us"' : ""}>${esc(tx(c, lang))}</td>`).join("")}</tr>`).join("")}</tbody></table></div><p class="more"><a class="btn btn-solid" href="${PATHS.quote[lang]}">${esc(I18N[lang].quote.eyebrow)}</a> <a class="btn" href="${PATHS.hosting[lang]}">${esc(I18N[lang].hosting.eyebrow)}</a></p></div></section>`;
}
...
export function renderPricing(lang) {
  ...
  return join([
    ...
    `<main id="main"><section class="hero hero-sm"><div class="wrap"><div class="tag">${esc(p.eyebrow)}</div><h1>${p.title}</h1><p class="lede">${esc(p.lede)}</p><p class="hud">${esc(p.hud)}</p></div></section>`,
    market(p, lang), tiers(p, lang), addons(p, lang), compare(p, lang), vat(p), terms(p, lang), faq(p), sources(p),
    `<section class="section"><div class="wrap"><div class="contact-box"><i class="corner c1"></i><i class="corner c2"></i><i class="corner c3"></i><i class="corner c4"></i><h2 class="contact-title">${p.ctaTitle}</h2><p class="lede">${esc(p.ctaLede)}</p><div class="cta-row"><a class="btn btn-solid" href="mailto:${BRAND_EMAIL}">${ICON.mail} ${esc(ui.contact.email)}</a><a class="btn" href="${PATHS.hub[lang]}#demos">${esc(ui.nav.demos)}</a></div></div></div></section></main>`,
