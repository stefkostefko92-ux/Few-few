...
import { boot, siteNav, siteFooter, ghost, contact, HUB_FONTS, BRAND_BG } from "./hub.mjs";
...
export function projectCard(lang, pr, ui, i, full) {
  ...
  const cover = pr.shot
    ? `<img class="pj-shot" src="/img/projects/${pr.id}.webp" alt="${esc(t.name)} — ${esc(t.category)}" width="960" height="600" loading="lazy" decoding="async">`
  ...
  return `<article class="cell pj reveal" data-cursor><a class="pj-cover" href="${pr.url}" target="_blank" rel="noopener" aria-label="${esc(p.visit)}: ${esc(t.name)}">${cover}<span class="live" aria-hidden="true">${esc(p.live)}</span></a><div class="pj-meta"><span class="mono-num">${String(i + 1).padStart(3, "0")}</span> <span class="cat">${esc(t.category)}</span><h3>${esc(t.name)}</h3><p>${esc(t.desc)}</p>${full ? `<ul class="pj-facts">${t.facts.map((f) => `<li>${ICON.check}${esc(f)}</li>`).join("")}</ul>` : ""}<div class="pj-stack" aria-label="${esc(p.stack)}">${pr.stack.map((s) => `<span>${esc(s)}</span>`).join("")}</div><div class="pj-links"><a class="pj-visit" href="${pr.url}" target="_blank" rel="noopener">${esc(p.visit)} ↗</a>${cs}${!pr.shot && full ? `<span class="pj-note">${esc(p.noShot)}</span>` : ""}</div></div></article>`;
}
