...
export function renderBrochure(lang) {
  ...
  const projects = `<section class="bp"><h2>${esc(b.projectsTitle)}</h2><ul class="bp-projects">${shots.map((pr) => { const t = pr.t[lang]; return `<li><img src="/img/projects/${pr.id}-sm.webp" alt="${esc(t.name)}" width="960" height="600"><b>${esc(t.name)}</b><span>${esc(t.category)}</span><small>${pr.url.replace(/^https?:\/\//, "")}</small></li>`; }).join("")}</ul>${foot(3)}</section>`;
