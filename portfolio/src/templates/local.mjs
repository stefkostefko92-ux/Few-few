...
export function renderLocalIndex(lang) {
  ...
  return join([
    ...
    `<body class="hub">`, boot(ui), siteNav(lang, ui, PATHS.local),
    `<main id="main"><section class="section" style="padding-top:140px"><div class="wrap"><div class="tag reveal">// ${esc(L.eyebrow)}</div><h1 class="h2 reveal">${L.indexH1}</h1><p class="lede reveal">${esc(L.indexLede)}</p><div class="grid1"><article class="cell reveal"><h3>${esc(L.bg)}</h3><ul class="city-list">${CITIES.filter((c) => c.country === "BG").map((c) => `<li><a href="${localPath(lang, c)}">${esc(c.name[lang])} →</a></li>`).join("")}</ul></article><article class="cell reveal"><h3>${esc(L.it)}</h3><ul class="city-list">${CITIES.filter((c) => c.country === "IT").map((c) => `<li><a href="${localPath(lang, c)}">${esc(c.name[lang])} →</a></li>`).join("")}</ul></article><article class="cell reveal"><h3>${esc(L.remote)}</h3><p>${esc(L.remoteText)}</p></article></div></div></section>`,
...
export function renderLocal(lang, c) {
  ...
  return join([
    ...
    `<body class="hub">`, boot(ui), siteNav(lang, ui, PATHS.local),
    `<main id="main"><section class="section" style="padding-top:140px"><div class="wrap"><nav class="crumbs tag" aria-label="breadcrumb"><a href="${PATHS.hub[lang]}">Portfolio</a> / <a href="${PATHS.local[lang]}">${esc(L.eyebrow)}</a> / ${esc(city)}</nav><h1 class="h2 reveal">${rep(L.h1)}</h1><p class="lede reveal">${esc(c.text[lang])}</p><p class="lede reveal">${esc(rep(L.lede))}</p><ul class="stats reveal" style="margin:0 0 48px;grid-template-columns:repeat(3,1fr)"><li><b>${money(shown(TIERS[0].price, lang), lang)}</b><span>${esc(ui.pricing.tiers.start.tag)}</span></li><li><b>${TIERS[0].days[0]}–${TIERS[1].days[1]}</b><span>${esc(ui.pricing.days)}</span></li><li><b>${DEMOS.length}</b><span>${esc(ui.demos.eyebrow)}</span></li></ul></div></section>`,
    `<section class="section alt"><div class="wrap"><div class="tag reveal">// ${esc(rep(L.demosTitle))}</div><div class="grid1">${picks.map((d) => `<a class="cell reveal" href="${demoPath(lang, d)}" data-cursor><span class="cat">${esc(d.t[lang].category)}</span><h3>${esc(d.t[lang].name)}</h3><p>${esc(d.t[lang].hero.proof)}</p><span class="open">${esc(ui.brand.open)}</span></a>`).join("")}</div><p class="more"><a class="btn" href="${PATHS.hub[lang]}#demos">${esc(ui.hero.ctaDemos)} ${ICON.arrow}</a></p></div></section>`,
    `<section class="section"><div class="wrap" style="max-width:820px"><div class="tag">// FAQ · ${esc(city)}</div>${L.faq.map((f, i) => `<details class="faq"${i === 0 ? " open" : ""}><summary><h3>${esc(rep(f.q))}</h3></summary><p style="padding:0 0 18px;font-size:12px;line-height:1.9;color:var(--text-2)">${esc(rep(f.a))}</p></details>`).join("")}<p class="tiny" style="margin-top:20px">${esc(L.mapNote)} <a href="https://www.openstreetmap.org/?mlat=${c.lat}&amp;mlon=${c.lng}#map=12/${c.lat}/${c.lng}" target="_blank" rel="noopener">OpenStreetMap ↗</a> · ${esc(BRAND_ADDRESS.street)}, ${BRAND_ADDRESS.zip} ${esc(BRAND_ADDRESS.city)}</p></div></section>`,
