...
export function renderLocalIndex(lang) {
  ...
  return join([
    ...
    `<main id="main"><section class="section" style="padding-top:140px"><div class="wrap"><div class="tag reveal">// ${esc(L.eyebrow)}</div><h1 class="h2 reveal">${L.indexH1}</h1><p class="lede reveal">${esc(L.indexLede)}</p><div class="grid1"><article class="cell reveal"><h2>${esc(L.bg)}</h2><ul class="city-list">${CITIES.filter((c) => c.country === "BG").map((c) => `<li><a href="${localPath(lang, c)}">${esc(c.name[lang])} →</a></li>`).join("")}</ul></article><article class="cell reveal"><h2>${esc(L.it)}</h2><ul class="city-list">${CITIES.filter((c) => c.country === "IT").map((c) => `<li><a href="${localPath(lang, c)}">${esc(c.name[lang])} →</a></li>`).join("")}</ul></article><article class="cell reveal"><h2>${esc(L.remote)}</h2><p>${esc(L.remoteText)}</p></article></div></div></section>`,
