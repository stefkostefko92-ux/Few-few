...
import { siteNav, siteFooter, boot, ghost, contact, HUB_FONTS, BRAND_BG } from "./hub.mjs";
...
export function renderHosting(lang) {
  ...
  const plans = h.plans.map((pl) => { const a = price(pl.price); return `<article class="tier${pl.popular ? " pop" : ""} reveal" data-cursor>${pl.popular ? `<span class="pop-badge">${esc(p.popular)}</span>` : ""}<h2>${esc(pl.name)}</h2><p class="tier-price"><strong>${money(shown(a.price, lang), lang)}</strong><span class="tier-market">${esc(p.monthly)} · ${esc(ui.pricing.hud)}</span></p><ul>${pl.items.map((i) => `<li>${ICON.check}<span>${esc(i)}</span></li>`).join("")}</ul><a class="btn ${pl.popular ? "btn-solid" : ""}" data-magnetic href="mailto:${BRAND_EMAIL}?subject=${encodeURIComponent(pl.name + " — Carbon Stealth")}">${esc(p.choose)} ${ICON.arrow}</a></article>`; }).join("");
  ...
  return join([
    ...
    `<body class="hub">`, boot(ui), siteNav(lang, ui, PATHS.hosting),
    `<main id="main"><section class="section" style="padding-top:140px;padding-bottom:40px"><div class="wrap"><div class="tag reveal">// ${esc(h.eyebrow)}</div><h1 class="h2 reveal">${h.h1}</h1><p class="lede reveal">${esc(h.lede)}</p><div class="tiers tiers-2">${plans}</div></div></section>`,
    `<section class="section alt"><div class="wrap">${ghost(esc(h.factsTitle))}<div class="grid1" style="margin-top:40px">${h.facts.map(([t, d], i) => `<article class="cell reveal" data-cursor><span class="mono-num">0${i + 1}</span><h3>${esc(t)}</h3><p>${esc(d)}</p></article>`).join("")}</div></div></section>`,
