...
export function renderHosting(lang) {
  ...
  const plans = h.plans.map((pl) => { const a = price(pl.price); return `<article class="tier${pl.popular ? " pop" : ""} reveal" data-cursor>${pl.popular ? `<span class="pop-badge">${esc(p.popular)}</span>` : ""}<h3>${esc(pl.name)}</h3><p class="tier-price"><strong>${money(shown(a.price, lang), lang)}</strong><span class="tier-market">${esc(p.monthly)} · ${esc(ui.pricing.hud)}</span></p><ul>${pl.items.map((i) => `<li>${ICON.check}<span>${esc(i)}</span></li>`).join("")}</ul><a class="btn ${pl.popular ? "btn-solid" : ""}" data-magnetic href="mailto:${BRAND_EMAIL}?subject=${encodeURIComponent(pl.name + " — Carbon Stealth")}">${esc(p.choose)} ${ICON.arrow}</a></article>`; }).join("");
