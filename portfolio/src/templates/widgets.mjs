...
const kinds = {
  ...
  tiles: (w, c, t, imgs = []) => `<div class="widget w-tiles" data-widget="tiles"${t.calc ? ` data-calc="${esc(JSON.stringify(t.calc))}"` : ""}><h2 class="w-title">${esc(w.title)}</h2><div class="tiles">${w.rows.map(([n, s, p], i) => `<button type="button" class="tile tile-${i % 3}" aria-pressed="false" data-price="${esc(p)}"><span class="tile-art" aria-hidden="true"${imgs[i] ? ` style="background-image:url(${imgs[i]})"` : ""}></span><strong>${esc(n)}</strong><span>${esc(s)}</span><em>${esc(p)}</em></button>`).join("")}</div><div class="w-cart" hidden><span><b data-count>0</b> ${esc(c.widget.items)}</span><strong data-total></strong><button type="button" class="w-clear">${esc(c.widget.clear)}</button></div></div>`,
  stats: (w, c, t) => `<div class="widget w-stats" data-widget="stats"${t.calc ? ` data-calc="${esc(JSON.stringify(t.calc))}"` : ""}><h2 class="w-title">${esc(w.title)}</h2><div class="kpis">${w.rows.map(([n, l, d]) => `<div class="kpi${d ? " kpi-" + d : ""}"><strong data-countup>${esc(n)}</strong><span>${esc(l)}</span></div>`).join("")}</div>${w.cta ? `<a class="btn btn-primary w-cta" href="#contact">${esc(w.cta)} ${ICON.arrow}</a>` : ""}</div>`,
...
/** @param w widget данни · c = общите UI низове (common) · t = съдържанието на демото за езика · imgs = снимки за плочките */
export function widget(w, c, t, imgs) {
  const fn = kinds[w.kind];
  if (!fn) throw new Error(`непознат widget kind: ${w.kind}`);
  return fn(w, c, t, imgs);
}
