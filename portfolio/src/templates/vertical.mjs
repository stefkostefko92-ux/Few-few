...
import { siteNav, siteFooter, contact, ghost, HUB_FONTS, BRAND_BG, LAB } from "./hub.mjs";
...
export function renderVertical(lang, demo) {
  ...
  const hero = `<section class="section" style="padding-top:140px"><div class="wrap"><nav class="crumbs" aria-label="breadcrumb"><a href="${PATHS.hub[lang]}">Portfolio</a> / <a href="${PATHS.vertical[lang]}">${esc(V.crumb)}</a> / ${esc(t.category)}</nav><h1 class="h2">${esc(v.h1)}</h1><p class="lede v-lede">${esc(rep(v.intro[0]))}</p><div class="cta-row"><a class="btn btn-solid" href="${demoPath(lang, demo)}">${esc(V.openDemo)}</a><a class="btn" href="${PATHS.quote[lang]}?demo=${demo.id}">${esc(rep(V.quote))}</a></div><ul class="stats"><li><b>${money(shown(start.price, lang), lang)}</b><span>${esc(V.stats.from)}</span></li><li><b>${start.days[0]}–${biz.days[1]}</b><span>${esc(V.stats.days)}</span></li><li><b>3</b><span>${esc(V.stats.langs)}</span></li>${lab ? `<li><b>${lab.mobile.score}/${lab.desktop.score}</b><span>${esc(V.stats.lab)}</span></li>` : ""}</ul></div></section>`;
  const demoBlock = `<section class="section alt" id="demo"><div class="wrap"><h2 class="sh">${esc(V.demoTitle)}</h2><p class="lede">${esc(V.demoLede)}</p><a class="v-shot" href="${demoPath(lang, demo)}" style="--c-bg:${demo.theme.bg}">${preview ? `<img src="${preview}" alt="${esc(t.name)} — ${esc(t.category)}" width="960" height="600" loading="lazy" decoding="async">` : `<span class="v-shot-type" aria-hidden="true">${DEMO_ICONS[demo.icon] || ""}</span>`}<span class="v-shot-cta">${esc(V.openDemo)} ${ICON.arrow}</span></a></div></section>`;
  const includesBlock = `<section class="section" id="includes"><div class="wrap">${ghost(esc(rep(V.includesTitle)))}<ul class="v-includes">${includes.map((i) => `<li >${ICON.check}<span>${esc(i)}</span></li>`).join("")}</ul></div></section>`;
  const priceBlock = `<section class="section alt" id="price"><div class="wrap">${ghost(esc(V.priceTitle))}<p class="lede">${esc(V.priceLede)}</p><div class="grid1 v-tiers">${[[start, V.tierStart], [biz, V.tierBusiness], [shop, V.tierEcommerce]].map(([tr, note]) => `<a class="cell" href="${PATHS.pricing[lang]}#${tr.id}"><span class="cat">${esc(ui.pricing.tiers[tr.id].tag)}</span><h3>${esc(ui.pricing.tiers[tr.id].name)}</h3><b class="v-price">${money(shown(tr.price, lang), lang)}</b><p>${esc(note)} · ${tr.days[0]}–${tr.days[1]} ${esc(ui.pricing.days)}</p><span class="open">${esc(ui.brand.open)}</span></a>`).join("")}</div><p class="more"><a class="btn" href="${PATHS.pricing[lang]}">${esc(V.allPrices)}</a> <a class="btn btn-solid" href="${PATHS.quote[lang]}?demo=${demo.id}">${esc(rep(V.quote))}</a></p></div></section>`;
  const whyBlock = `<section class="section" id="why"><div class="wrap" style="max-width:820px"><h2 class="sh">${esc(rep(V.whyTitle))}</h2><p class="lede">${esc(rep(v.intro[1]))}</p></div></section>`;
  const faqBlock = `<section class="section alt" id="faq"><div class="wrap" style="max-width:820px"><h2 class="sh">${esc(rep(V.faqTitle))}</h2>${faqs.map((f, i) => `<details class="faq"${i === 0 ? " open" : ""}><summary><h3>${esc(f.q)}</h3></summary><p style="padding:0 0 18px;font-size:12px;line-height:1.9;color:var(--text-2)">${esc(f.a)}</p></details>`).join("")}</div></section>`;
  const relatedBlock = `<section class="section" id="related"><div class="wrap"><h2 class="sh">${esc(V.relatedTitle)}</h2><div class="grid1 grid1-4">${relatedFilled.map((d) => `<a class="cell" href="${verticalPath(lang, d)}"><span class="ic">${DEMO_ICONS[d.icon] || ""}</span><h3>${esc(VERTICALS[lang][d.id].h1)}</h3><p>${esc(d.t[lang].name)} · ${esc(d.t[lang].category)}</p><span class="open">${esc(ui.brand.open)}</span></a>`).join("")}</div><p class="more"><a class="btn" href="${PATHS.vertical[lang]}">${esc(V.eyebrow)}</a></p></div></section>`;
  const moreBlock = `<section class="section alt" id="more"><div class="wrap"><div class="grid1 grid1-2"><div class="cell"><h3 class="sh">${esc(V.blogTitle)}</h3><ul class="city-list">${articles.map((a) => `<li><a href="${articlePath(lang, a)}">${esc(a.t[lang].title)} →</a></li>`).join("")}</ul></div><div class="cell"><h3 class="sh">${esc(V.citiesTitle)}</h3><ul class="city-list">${cities.map((c) => `<li><a href="${localPath(lang, c)}">${esc(c.name[lang])} →</a></li>`).join("")}</ul></div></div></div></section>`;
  return join([
    ...
    `<body class="hub">`, siteNav(lang, ui, paths),
...
export function renderVerticalIndex(lang) {
  ...
  return join([
    ...
    `<body class="hub">`, siteNav(lang, ui, PATHS.vertical),
    `<main id="main"><section class="section" style="padding-top:140px"><div class="wrap"><div class="tag">${esc(V.eyebrow)}</div><h1 class="h2">${V.indexH1}</h1><p class="lede">${esc(rep(V.indexLede))}</p><div class="grid1">${DEMOS.map((d) => { const v = VERTICALS[lang][d.id], lab = LAB?.pages?.[d.id]; return `<a class="cell" href="${verticalPath(lang, d)}"><span class="ic">${DEMO_ICONS[d.icon] || ""}</span><span class="cat">${esc(d.t[lang].category)}</span><h2>${esc(v.h1)}</h2><p>${esc(rep(v.desc))}</p><span class="open">${esc(ui.brand.open)}${lab ? ` · Lighthouse ${lab.mobile.score}/${lab.desktop.score}` : ""}</span></a>`; }).join("")}</div></div></section>`,
