...
export function createMaterials(T, palette) {
  ...
  const jelly = withRimGlow(
    new THREE.MeshPhysicalMaterial({
      ...
      normalScale: v2(0.035), // barely visible — sealed inside, not printed on top
  ...
  const coreGlow = new THREE.MeshBasicMaterial({ name: 'core', color: p.olive, map: T.core, toneMapped: false, transparent: true, opacity: 0.85 });
  ...
  const lens = new THREE.MeshPhysicalMaterial({ name: 'lens', color: 0xffffff, transmission: 0.95, roughness: 0.1, ior: 1.5, thickness: 0.05, envMapIntensity: 0.16, clearcoat: 0.1, clearcoatRoughness: 0.25 });

  const sclera = new THREE.MeshStandardMaterial({ name: 'sclera', color: p.eye, roughness: 0.42, envMapIntensity: 0.16 });
  const iris = new THREE.MeshStandardMaterial({ name: 'iris', color: p.inkSoft, roughness: 0.4, envMapIntensity: 0.35 });
  ...
  const satin = new THREE.MeshPhysicalMaterial({ name: 'satin', color: p.inkSoft, roughness: 0.36, sheen: 1, sheenRoughness: 0.28, sheenColor: new THREE.Color(p.pale), normalMap: T.satin.normalMap, normalScale: v2(0.5), envMapIntensity: 1.1 });
