...
test("localeOf / countryOf: само краен речник влиза в ClickEvent", () => {
  assert.equal(localeOf("bg"), "bg");
  assert.equal(localeOf("<script>"), undefined);
  assert.equal(localeOf(null), undefined);
  assert.equal(countryOf("bg"), "BG");
  assert.equal(countryOf("XX"), undefined);
  assert.equal(countryOf("T1"), undefined);
  assert.equal(countryOf("BGR"), undefined);
