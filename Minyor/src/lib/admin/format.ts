...
// Булеви полета, които при нов запис по подразбиране са изключени.
const DEFAULT_OFF = new Set(["isHome", "isOwnTeam"]);
...
export function buildInitial(
  ...
  for (const field of resource.fields) {
    ...
    if (field.type === "boolean") {
      // „published" и „active" са включени по подразбиране; маркерите за
      // домакин/наш отбор — изключени.
      out[field.name] = !DEFAULT_OFF.has(field.name);
