...
// Числови полета, които при празна стойност остават NULL (а не 0).
...
export async function saveRecord(
  ...
  // Сървърна проверка на задължителните полета.
  for (const field of resource.fields) {
    if (field.required) {
      const raw = formData.get(field.name);
...
export async function togglePublish(
  ...
  if (next && resource.fields.some((f) => f.name === "publishedAt")) {
    const row = await delegate(resource.model).findUnique({ where: { id } });
