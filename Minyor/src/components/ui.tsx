...
export function PageHero({
  ...
  return (
    <div className="page-hero relative overflow-hidden border-b border-slate-200 bg-gradient-to-br from-brand-50 via-white to-white">
      {/* Гербът като дискретен воден знак */}
      <Crest
        decorative
        className="pointer-events-none absolute -right-2 -top-4 hidden h-44 w-auto opacity-[0.06] sm:block"
      />
      <div className="container-content relative py-9 sm:py-11">
        {crumbs && crumbs.length > 0 && (
          <>
            <Breadcrumbs crumbs={[{ name: "Начало", path: "/" }, ...crumbs]} />
            <JsonLd data={breadcrumbLd([{ name: "Начало", path: "/" }, ...crumbs])} />
          </>
        )}
      ...
      <ol className="flex flex-wrap items-center gap-1">
        {crumbs.map((c, i) => (
          <li key={c.path} className="flex items-center gap-1">
            ...
            {i < crumbs.length - 1 ? (
              <Link href={c.path} className="hover:text-brand-800">
                {c.name}
              </Link>
            ) : (
...
export function EmptyState({ title, hint }: { title: string; hint?: string }) {
  return (
    <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-gradient-to-b from-brand-50/60 to-white p-12 text-center sm:p-16">
      <div className="mx-auto mb-4 grid h-20 w-20 place-items-center rounded-full bg-brand-100 text-brand-700 shadow-inner">
    ...
    <section className="container-content py-10">
      <div className="mb-6 flex items-end justify-between gap-4">
        ...
        {href && (
          <Link
            href={href}
            className="shrink-0 text-sm font-semibold text-brand-800 hover:text-brand-900 hover:underline"
          >
            {hrefLabel} →
          </Link>
        )}
