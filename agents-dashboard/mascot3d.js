...
function createMaterials(T, palette) {
  ...
  const acetate = new THREE6.MeshPhysicalMaterial({
    name: "acetate",
    color: p.ink,
    roughness: 0.42,
    clearcoat: 0.4,
    ...
    normalMap: T.scratch.normalMap,
    normalScale: v2(0.12),
    roughnessMap: T.scratch.roughnessMap
  });
  const lens = lensGlassMaterial(p);
  const catchlight = new THREE6.MeshBasicMaterial({ name: "catchlight", map: T.radial, color: 16777215, transparent: true, opacity: 0.3, depthWrite: false, blending: THREE6.AdditiveBlending });
  ...
  const browFuzz = new THREE6.MeshStandardMaterial({ name: "browFuzz", color: p.ink, roughness: 0.82, normalMap: T.felt.normalMap, normalScale: v2(0.35), envMapIntensity: 0.25 });
  ...
  const satin = new THREE6.MeshPhysicalMaterial({
    name: "satin",
    color: p.inkSoft,
    roughness: 0.46,
    sheen: 1,
    sheenRoughness: 0.35,
    sheenColor: new THREE6.Color(p.pale),
    normalMap: T.satin.normalMap,
    normalScale: v2(0.5),
    roughnessMap: T.satin.roughnessMap,
    envMapIntensity: 0.35,
    clearcoat: 0
  });
  const satinKnot = satin.clone();
  satinKnot.color = new THREE6.Color(p.inkSoft);
  const bubble = new THREE6.ShaderMaterial({
    name: "bubble",
    transparent: true,
    depthWrite: false,
  ...
  const shade = new THREE6.MeshPhysicalMaterial({ name: "shade", color: 4863e3, roughness: 0.5, side: THREE6.DoubleSide, transmission: 0.65, thickness: 0.25, ior: 1.3, envMapIntensity: 0.4, emissive: 3810320, emissiveIntensity: 0.3 });
  const windowGlass = new THREE6.MeshBasicMaterial({ name: "windowGlass", map: T.windowSky, toneMapped: false });
  const windowFrame = new THREE6.MeshStandardMaterial({ name: "windowFrame", color: 1445642, roughness: 0.7, envMapIntensity: 0.3 });
  const dust = new THREE6.MeshBasicMaterial({ name: "dust", color: 15918784, map: T.radial, transparent: true, opacity: 0.22, depthWrite: false, blending: THREE6.AdditiveBlending });
  const dustSprite = new THREE6.SpriteMaterial({ color: 15918784, map: T.radial, transparent: true, opacity: 0.16, depthWrite: false, blending: THREE6.AdditiveBlending });
  return {
    jelly,
    limb,
    fabric,
...
function scratchTextures(size = 128, streaks = 40) {
  const n = size * size;
  const H = new Float32Array(n);
  const R = new Float32Array(n);
  const rand = rng(83);
  const lines = Array.from({ length: streaks }, () => ({ y: rand(), depth: 0.15 + rand() * 0.5, width: 4e-3 + rand() * 0.01, slope: (rand() - 0.5) * 0.3 }));
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const i = y * size + x;
      const u = x / size;
...
function leatherTextures(size = 96) {
  const n = size * size;
  const H = new Float32Array(n);
  const R = new Float32Array(n);
...
function windowSkyTexture(size = 192) {
  const n = size * size;
  const Al = new Uint8Array(n * 4);
  const rand = rng(207);
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      for (const [lx, ly, lb] of lights) {
        ...
        const glow = (1 - smooth(0, 0.045, d)) * lb;
        r += 235 * glow;
        g += 200 * glow;
        b += 120 * glow;
      }
...
function buildBody(materials, textures) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const b = new THREE8.Mesh(bubbleGeo, materials.bubble);
    b.scale.setScalar(r);
...
var BROW_TUBE_R = 0.052;
var BROW_FLATTEN_Y = 0.74;
function brow(sign, materials) {
  const z0 = bodyRadiusAtY(BROW_Y) * 1.012;
  const curve = new THREE9.CatmullRomCurve3([
    new THREE9.Vector3(-0.115, -6e-3, 0),
    new THREE9.Vector3(-0.05, 0.024, 3e-3),
    new THREE9.Vector3(0, 0.03, 4e-3),
    new THREE9.Vector3(0.05, 0.019, 2e-3),
    new THREE9.Vector3(0.115, -0.014, 0)
  ]);
  const geo = new THREE9.TubeGeometry(curve, 24, BROW_TUBE_R, 12, false);
  const b = new THREE9.Mesh(geo, materials.browFuzz);
  b.scale.set(1, BROW_FLATTEN_Y, 0.82);
...
function bowWing(sign, materials) {
  ...
  shape.closePath();
  const geo = new THREE10.ExtrudeGeometry(shape, { depth: WING_DEPTH, bevelEnabled: true, bevelThickness: 0.02, bevelSize: 0.018, bevelSegments: 8, curveSegments: 24 });
...
function buildBow(materials) {
  ...
  const knot = new THREE10.Mesh(new THREE10.CylinderGeometry(0.05, 0.055, 0.1, 20), materials.satinKnot);
...
var DESK_TOP_H = 0.14;
var DESK_Y = GROUND_Y - DESK_TOP_H / 2;
var LAMP_POS = new THREE11.Vector3(1.35, GROUND_Y + 1.22, -0.35);
function desktop(materials) {
  const group = new THREE11.Group();
  const top = new THREE11.Mesh(new THREE11.BoxGeometry(6.4, DESK_TOP_H, 3.8), materials.wood);
  top.position.y = DESK_Y;
  top.receiveShadow = true;
  top.castShadow = false;
  group.add(top);
  const legGeo = new THREE11.BoxGeometry(0.16, 1.3, 0.16);
  for (const [x, z] of [[-3, 1.7], [3, 1.7], [-3, -1.7], [3, -1.7]]) {
    const leg2 = new THREE11.Mesh(legGeo, materials.wood);
    leg2.position.set(x, DESK_Y - DESK_TOP_H / 2 - 0.65, z);
...
function lamp(materials) {
  const group = new THREE11.Group();
  const top = DESK_Y + DESK_TOP_H / 2;
  const base = new THREE11.Mesh(new THREE11.CylinderGeometry(0.22, 0.27, 0.06, 24), materials.brass);
  base.position.set(LAMP_POS.x, top + 0.03, LAMP_POS.z);
  base.castShadow = true;
  group.add(base);
  const pole = new THREE11.Mesh(new THREE11.CylinderGeometry(0.035, 0.04, 1.1, 12), materials.brass);
  pole.position.set(LAMP_POS.x, top + 0.06 + 0.55, LAMP_POS.z);
  pole.castShadow = true;
  group.add(pole);
  const shade = new THREE11.Mesh(new THREE11.CylinderGeometry(0.26, 0.42, 0.46, 22, 1, true), materials.shade);
  shade.position.copy(LAMP_POS);
  shade.rotation.x = Math.PI;
  group.add(shade);
  const ring = new THREE11.Mesh(new THREE11.TorusGeometry(0.42, 0.015, 8, 20), materials.brass);
  ring.position.set(LAMP_POS.x, LAMP_POS.y - 0.22, LAMP_POS.z);
  ring.rotation.x = Math.PI / 2;
  group.add(ring);
  const bulb = new THREE11.Mesh(new THREE11.SphereGeometry(0.11, 12, 10), materials.bulb);
  ...
  group.add(light);
  const haze = new THREE11.Mesh(
    new THREE11.ConeGeometry(0.5, 1.1, 20, 1, true),
    new THREE11.MeshBasicMaterial({ color: 16764830, transparent: true, opacity: 0.025, depthWrite: false, blending: THREE11.AdditiveBlending, side: THREE11.FrontSide, toneMapped: false })
  );
...
function window_(materials) {
  const group = new THREE11.Group();
  const pane = new THREE11.Mesh(new THREE11.PlaneGeometry(4.4, 2.7), materials.windowGlass);
  pane.position.set(-0.4, 0.75, -2.8);
  group.add(pane);
  const barGeo = new THREE11.BoxGeometry(4.6, 0.08, 0.08);
  const bar = new THREE11.Mesh(barGeo, materials.windowFrame);
  bar.position.set(-0.4, 0.75, -2.78);
  group.add(bar);
  const postGeo = new THREE11.BoxGeometry(0.08, 2.9, 0.08);
  const post = new THREE11.Mesh(postGeo, materials.windowFrame);
...
function dustField(materials, count = 14) {
  ...
  for (let i = 0; i < count; i++) {
    const s = new THREE11.Sprite(materials.dustSprite.clone());
...
function addLights(scene, p) {
  ...
  scene.add(underFill);
  const overhead = new THREE12.DirectionalLight(new THREE12.Color(p.gold), 0.6);
  overhead.position.set(0.6, 5, 0.6);
  scene.add(overhead);
  scene.add(new THREE12.HemisphereLight(1845056, 2758406, 0.55));
}
function softStudioEnvironment(renderer, p) {
  const envScene = new THREE12.Scene();
  const sky = new THREE12.Mesh(
    new THREE12.SphereGeometry(20, 16, 12),
    new THREE12.ShaderMaterial({
      side: THREE12.BackSide,
      uniforms: { top: { value: new THREE12.Color(1318160) }, bottom: { value: new THREE12.Color(131842) } },
  ...
  const panel = (x, y, z, ry, w, h, color, intensity) => {
    const m = new THREE12.Mesh(new THREE12.PlaneGeometry(w, h), new THREE12.MeshBasicMaterial({ color, toneMapped: false }));
...
function buildScene(renderer, palette = PALETTE) {
  ...
  scene.background = new THREE12.Color(p.bg);
  ...
  const mascot = new THREE12.Group();
  ...
  const glow = new THREE12.Mesh(new THREE12.PlaneGeometry(2.4, 2.4), materials.glow);
  glow.rotation.x = -Math.PI / 2;
  glow.position.y = GROUND_Y + 2e-3;
  scene.add(glow);
  const caustic = new THREE12.Mesh(new THREE12.PlaneGeometry(2, 2), materials.caustic);
...
function mount(el, opts = {}) {
  ...
  const renderer = new THREE13.WebGLRenderer({ canvas, antialias: false, alpha: true, powerPreference: "low-power" });
  renderer.toneMapping = THREE13.NoToneMapping;
  renderer.outputColorSpace = THREE13.LinearSRGBColorSpace;
  renderer.shadowMap.enabled = true;
  renderer.shadowMap.type = THREE13.PCFSoftShadowMap;
  const palette = tintPalette(opts.accent);
  const { scene, animate } = buildScene(renderer, palette);
  scene.background = null;
  const CAM_DIST = 11;
  const camera = new THREE13.PerspectiveCamera(26, 1, 0.1, 40);
  ...
  ro.observe(el);
  resize();
  const pointer = new THREE13.Vector2(0, 0);
  const onMove = (e) => {
