  ...
  if (hits.length === 0) {
    ...
    return {
      answer:
        ...
        "информация. Междувременно може да потърсите помощ на гише или да ни " +
...
async function answerWithClaude(
  ...
  const system =
    ...
    `информацията липсва, кажи честно, че не разполагаш с нея, и насочи към ` +
    `помощ на гише. Не измисляй телефони и адреси.\n\n` +
    `ЛОКАЛЕН КОНТЕКСТ:\n${context}`;
