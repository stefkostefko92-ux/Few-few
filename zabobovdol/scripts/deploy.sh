...
# Тайните в .env — само за собственика (стари инсталации го имат 644).
chmod 600 .env 2>/dev/null || true

# Нестандартен порт = зад друг reverse proxy на хоста. Тогава портът трябва да
# слуша само локално, иначе сайтът е достъпен и направо по IP:порт — без HTTPS и
# покрай заглавията на хостовия прокси.
HTTP_BIND="$(read_env HTTP_BIND)"
if [ "$HTTP_PORT" != "80" ] && [ "${HTTP_BIND:-0.0.0.0}" = "0.0.0.0" ]; then
  warn "ВНИМАНИЕ: порт $HTTP_PORT слуша на всички адреси (0.0.0.0)."
  warn "  Ако пред сайта стои Nginx на хоста, добавете в .env:  HTTP_BIND=127.0.0.1"
fi

# Място на диска: билдът на образа (Next.js + Prisma) иска няколко GB. Пълен диск
# по средата на `up --build` оставя полуизграден образ и спрян сайт.
FREE_KB="$(df -Pk . 2>/dev/null | awk 'NR==2{print $4}')"
if [ -n "$FREE_KB" ] && [ "$FREE_KB" -lt 3145728 ]; then
  warn "Свободни са само $((FREE_KB / 1024)) MB (нужни са поне 3 GB) — спирам преди билда."
  warn "  Освободете място, напр.:  docker builder prune -f && docker image prune -f"
  exit 1
fi
