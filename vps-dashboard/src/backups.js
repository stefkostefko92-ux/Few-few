      ...
      shell: [
        ...
        unit
          ? [
              ...
              `systemctl stop ${unit}`,
              // Между stop и start има `set -e` и стъпки, които могат да паднат
              // (пълен дял, счупена цел). Без trap провалът оставя ПРОДУКТА
              // ДОЛУ — по-лошо състояние от преди възстановяването, и то без да
              // го пише никъде. Изричното пускане отдолу маха капана.
              `trap 'echo "⚠ Прекъснато — връщам ${unit}"; systemctl start ${unit} >/dev/null 2>&1 || true' EXIT`,
        ...
        `mkdir -p ${DUMP_DIR}`,
        // ИЗРИЧЕН if, не `A && B && C || echo`. При втората форма `||` хваща
        // провала на ВСЯКА брънка от веригата, не само на `[ -f ]`: заключена от
        // жив процес база връща „database is locked", изходът лъже „(няма текущ
        // файл)" и следващият ред презаписва данните БЕЗ точка за връщане.
        `if [ -f "${dst}" ]; then`,
        `  sqlite3 "${dst}" ".backup '${DUMP_DIR}/${stamp}.sqlite'" || { echo "Защитната снимка се провали (заключена база?) — СПИРАМ, преди да пипна данните."; exit 1; }`,
        `  gzip -f "${DUMP_DIR}/${stamp}.sqlite"`,
        `  [ -s "${DUMP_DIR}/${stamp}.sqlite.gz" ] || { echo "Защитната снимка е празна — СПИРАМ."; exit 1; }`,
        `else echo "(няма текущ файл — няма какво да се пази)"; fi`,
        `echo "▸ Възстановявам…"`,
        `cp "${src}" "${dst}"`,
