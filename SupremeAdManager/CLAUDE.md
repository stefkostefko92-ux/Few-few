...
7. **Многоезичен UI (bg/en/it/de/es)** през `src/i18n.js` + `src/locales/*.json` —
   **българският е източникът на истината**; нов низ = ключ в `bg.json` + превод във
   всичките 4 (агент „Преводач“); тестът за паритет на ключовете пази от дупки.
   **Одитната следа и предпазителите говорят САМО български** (правна прецизност) —
   никога не превеждай guard съобщения или audit записи. `/privacy` и `/data-deletion`
   са двуезични BG/EN правни страници — извън i18n системата (не се превеждат машинно).
