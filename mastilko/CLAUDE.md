...
- **Диктовка (глас→текст)** в CV/писмо (`TextTools.tsx`) ползва браузърния Web
  Speech API — по действие на потребителя, само ако браузърът го поддържа.
  **Правен контекст (одит 2026-07):** при Chrome звукът напуска устройството и
  се обработва от Google LLC (САЩ); понеже CV/писмо съдържат лични данни,
  разкритието изрично предупреждава, че продиктуваното (вкл. лични данни) отива
  към доставчика на браузъра (симетрично на AI бутоните) — inline бележка +
  раздел в `/poveritelnost`. **Не отслабвай тези текстове.** Приет остатъчен
  риск (собственик): защитима не-контролер стойка (услуга на браузъра,
  user-initiated, native mic-prompt), но надзорен орган би могъл да приеме
  (съвместен) контрол. PWA service-worker кешът е локален, само публични
  ресурси, `/api`+`/admin` изключени → ePrivacy чл. 5(3) изключение „строго
  необходимо“, без банер за съгласие.
...
- Общи парчета: `AiAssist` (AI бутон + предложения; режим `translate-en` за
  ...
  пренебрегва, несъвместим запис се мести в копията вместо да се презапише; 4-ти параметър `fresh` — подразбирания „от днес“ само при първо
  посещение, напр. текущият месец в календара; в `initial` не може — SSR), `ProjectFile` (свали/качи JSON + `ShareButton`),
  ...
  услуга), `FitText` (едър текст по листа — смалява се след рендер, докато и
  най-дългата дума се събере; иначе „АДМИНИСТРАЦИЯТА“ се реже при печат),
  `FitHeight` (блок с фиксирана височина — меню, обява, етикет — смалява
  текста чрез `--sheet-scale`, вместо да излиза от листа; при `onOverflow`
  PrintBar предупреждава), `useFitWidth` (прегледът „отблизо“ се смалява до
  ширината на картата, вместо да се скролва настрани на телефон).
  **`maxLength` на всяко поле ≤ `.max()` в `ProjectSchema`** — по-голям губи
  въведеното при презареждане; пази го `studio-inputs.test.ts`. Тествана логика: `src/lib/{print,vcard,wifi}.ts`;
...
## MCP конектор (ChatGPT и Claude)

`/api/mcp` е публичен отдалечен MCP сървър (Streamable HTTP, **без сесии и без
автентикация**): асистентът вика инструмент, ние връщаме **линк** към сайта с
попълнен дизайн — нищо не се създава и не се пази на сървъра. Говори **две
епохи**: текущата ревизия 2026-07-28 (без ръкостискане, `server/discover`,
огледални хедъри, `resultType`) И наследените 2025-06-18 и по-стари, които
ChatGPT и Claude ползват днес — не махай нито едната. Код в
`src/lib/mcp/{catalog,registry,server}.ts` + `src/app/api/mcp/route.ts`.
**Пълният контекст и обосновките на нетипичните решения са в `MCP.md` — чети
го, преди да пипаш конектора.** Накратко:
- **Без официалния SDK** — той иска Zod 4, продуктът е на Zod 3.
- **Границите на полетата са ОГЛЕДАЛО на `ProjectSchema` в студията.** По-широка
  граница тук = `parse` хвърля в `useLocalState` = потребителят отваря ПРАЗЕН
  редактор. Пази го тестът „границите НЕ надвишават схемите на студията“.
- **Маршрутът не чете бисквитки** — това е инвариантът, който прави пропускането
  на проверката за `Origin` безопасно. Добави ли се автентикация, проверката
  става задължителна.
- Това е вторият път (след AI бутоните), по който потребителско съдържание
  стига до наш сървър → разкрито е в `/poveritelnost`; **не отслабвай текста**.
