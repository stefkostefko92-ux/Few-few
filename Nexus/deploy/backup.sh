#!/bin/sh
# Nexus Dominion — SQLite hot backup.
#
# Usage:
#   deploy/backup.sh                     # writes to ./backups/nexus-dominion-YYYYMMDD-HHMMSS.db
#   deploy/backup.sh /path/to/out.db     # writes to a specific file
#
# When the app runs in Docker, run this inside the container, e.g.:
#   docker compose exec nexus-dominion /app/deploy/backup.sh
#
# Or on the host (with the volume mounted), point DB_PATH to the file.

set -e

DB_PATH="${DB_PATH:-/app/data/nexus-dominion.db}"
if [ ! -f "$DB_PATH" ]; then
...
if [ -z "$OUT" ]; then
  mkdir -p backups
  TS=$(date +%Y%m%d-%H%M%S)
  OUT="backups/nexus-dominion-$TS.db"
fi
