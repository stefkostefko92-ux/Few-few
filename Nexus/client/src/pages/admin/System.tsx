...
export function Broadcast(): React.ReactElement {
  ...
  const ok = from.trim() && subject.trim() && body.trim();

  async function send(e: React.FormEvent) {
    e.preventDefault();
    if (!ok) return;
    const c = await confirm({ title: t('broadcast.confirmTitle'), body: t('broadcast.confirmBody', { count: players ?? 0 }), confirmLabel: t('broadcast.send'), danger: true });
    if (!c) return;
    setSending(true);
    try {
      const r = await api.post('/admin/broadcast', { from_name: from.trim(), subject: subject.trim(), body: body.trim() });
      toast(t('broadcast.sent', { count: r.sent }), 'success');
      setSubject(''); setBody('');
    } catch (err) { toast(errMsg(err), 'error'); }
  ...
  return (
    <>
      <PageHeader title={t('broadcast.title')} subtitle={t('broadcast.intro')} />
      <form className="adm-card adm-form narrow" onSubmit={send}>
        ...
        <Field label={t('broadcast.body')} hint={t('broadcast.chars', { count: body.length, max: 2000 })} wide>{(id) => <textarea id={id} rows={10} value={body} maxLength={2000} onChange={(e) => setBody(e.target.value)} />}</Field>
        <div className="adm-form-actions">
          <button type="submit" className="btn btn-primary" disabled={sending || !ok}>{sending ? t('broadcast.sending') : t('broadcast.send')}</button>
        </div>
      </form>
    </>
  );
