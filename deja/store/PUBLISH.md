...
- Data usage → **„This item does not collect or use user data“** — не отмятай
  никоя категория (нищо не се предава извън устройството; локалният индекс не е
  „collection“ по дефиницията на CWS). Виж бележката по-долу.
- Certifications → отметни и трите.
...
### Бележка за Data Disclosure

CWS дефинира „collect“ като предаване на данни **извън устройството** към
разработчика или трета страна. Déjà не предава нищо: текстът, векторите,
откъсите и активният URL остават в IndexedDB / `storage.session` на потребителя;
единствената мрежова заявка е тегленето на моделни файлове (данни) от Hugging
Face, разкрито в политиката. Затова „does not collect“ е коректното и защитимо
деклариране. Ако ревюърът попита — отговорът е абзацът „The only network
request“ от описанието + `/privacy`.
