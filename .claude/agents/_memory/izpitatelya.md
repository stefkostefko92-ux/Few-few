...
- **2026-07-19:** Тествай контракта, не source: драйвай през публичния control plane, assert-вай наблюдаеми ефекти (върнати стойности, персистирано състояние, emit-нати събития); никога private полета/call order/backdoor „за тест". _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Усетена нужда от backdoor/reach-into-private = сигнал за грешен seam; поправи ДИЗАЙНА, не теста. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Едно поведение на it; името кодира три неща — поведение + условие + изход; „and" в името = дължиш split; „works"/„handles auth correctly" са невалидни; describe носи отговорността, не името на класа. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Rig (rig()/createHost()) връща най-малката нужна повърхност; само wiring, нула assert-и вътре; тестовете посягат в него, не пресъздават света. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Стъбвай само истинската външна граница: remote/service → spy на контракт-метода; network → на границата не вътрешностите; env/config → задай ПРЕДИ построяване на SUT, възстанови в afterEach; default са реални колаборатори. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Време/таймери → документирани knobs (инжектиран clock/manual tick/env), НЕ fake timers и НЕ реален setTimeout — fake timers патчват глобал вместо документиран seam. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** DAMP над DRY: литерални очаквани стойности (expect(x).toBe(15)), НЕ преизчислени с логиката на кода (toBe(price*rate) огледалва импл.); ключови предусловия вътре в it/rig; beforeEach само за cross-cutting plumbing. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Assert-вай само релевантния ефект, доказващ контракта; partial-object matcher за incidental полета; не assert-вай вътрешни броячи/call order/shapes. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Изолация нула-flakes: afterEach restore-ва всеки mock/spy + всеки env var + dispose host; нула зависимост от wall-clock/run-order/leftover on-disk; всеки сценарий own изолирана идентичност. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** CCCR bar: Clarity (провалът се чете сам), Completeness (success/error/boundary), Conciseness (един сценарий/it), Resilience (преживява вътрешен refactor без промяна на теста). _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Review mode = същите правила като чеклист в фиксиран ред (контракт→едно-поведение→boundary→DAMP→изолация→CCCR); докладвай evidence + fix с file:line + правилото; не пренаписвай без покана. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
- **2026-07-19:** Резолвай колаборатори по контракт (интерфейс + идентификатор), не по модула, който бинда конкретната имплементация — прави теста refactor-устойчив. _("тестова методология CCCR"; verified; "MoonshotAI/kimi-code:.agents/skills/agent-core-review/test/SKILL.md")_
