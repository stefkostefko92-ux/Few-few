...
## v9.0 — Стълбата на увереността: кога имаш право да кажеш „това работи"
- **`insights --brand slug` вече дава ИЗВОД, не само числа.** По подразбиране печата четимо резюме с три
  реда — **Кога** (пояс от деня) · **Формат** (IMAGE/REELS) · **Тема** (стълбът на плана) — и под всеки ред
  кофите с **медиана на ангажираността** и **брой постове**. Суровият отговор (всеки пост + дневен тренд) е
  зад `--json`.
- **Мярката е ангажираност/обхват, не суров обхват.** Обхватът расте заедно с последователите, тоест пост от
  третия месец бие пост от първия просто защото акаунтът е пораснал — класация по обхват мери РАСТЕЖА, не
  съдържанието. (Точно този дефект живя в избора на формат, докато не беше поправен.)
- **Медиана, не средно.** Един вирусен пост дърпа средното и обявява победител там, където има изключение.
- **Три степени, не две.** `none` (няма данни) · `early` (има, но не стигат) · `ready` (извод). Победител се
...
- **Без данни няма учене — това не е недостатък, а положението.** Преди първата публикация на акаунта
  НЯМА как да се знае кога неговата публика е будна: няма нито един собствен пост. Тогава се тръгва от
  плана на бранда (часове, зададени от човек) и се казва открито, че това са начални стойности, които
  първите ~6 поста на пояс ще проверят. Изследователско число за „най-добър час в Instagram" не е извод за
  ТОЗИ акаунт — цитирай го като външен ориентир със източник или не го цитирай.
