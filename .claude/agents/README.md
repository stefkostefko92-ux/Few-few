...
| **Рекламчика** `reklamchika.md`           | **Automated paid advertising** at enterprise grade — **Google Ads** (Search / PMax / **Demand Gen = YouTube** incl. Shorts / Display; API **v24**, REST, GAQL, micros, atomic Mutate with temp IDs, **PAUSED-first**, `Consent{ad_user_data, ad_personalization}`) + **Meta Ads** (Graph **v25.0**, ODAX objectives, `special_ad_categories`, placements incl. **`threads`**, **Click-to-WhatsApp** (`WHATSAPP_MESSAGE` CTA), `dsa_payor`/`dsa_beneficiary` for the EU, Advantage+ `targeting_automation`, **CAPI + pixel dedup** by `event_id`, BUC rate limits). Campaign strategy: learning-phase math ((CPA×50)/7), consolidation, +20% scaling steps, creative fatigue (freq >3.5), micro-budget→retargeting. EU-law hard blocks: Consent Mode v2, DSA arts. 26/28 (no <18 profiling, no art. 9 targeting), AI Act art. 50 disclosure; no political ads at all. Owns the **`SupremeAdManager/`** product (guarded, PAUSED-first, dry-run simulator, audit trail); `ads-lint.mjs` flags hardcoded tokens, ACTIVE-at-create, teen profiling, missing DSA/`special_ad_categories`/`event_id`. **The budget is sacred: nothing spends without a human click.** |
...
The original agents have reached **v10.0** — mastery — through genuine, source-verified learning
(see the version scheme below); the twentieth, **Рекламчика** (born 2026-07-12), is climbing with
16 verified lessons from its birth research and audits. Each definition layers: an **operating contract** (v1.0 —
...
matcher to the twenty agent types; the schema + `learn`-block format live in `.claude/agents/_memory/PROTOCOL.md`.
...
without `allowed_mentions`), `tools/ads/` (ads-lint.mjs: flags hardcoded ad-platform tokens,
campaigns created ACTIVE instead of PAUSED, teen profiling (`age_min < 18`), missing
`special_ad_categories`/`dsa_payor`/CAPI `event_id`, budgets not in micros, tight retry loops — Google
