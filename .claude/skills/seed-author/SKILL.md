...
```bash
node tools/seed/check-dups.mjs zabobovdol/prisma   # от корена; брой ПРЕДИ и СЛЕД промяната ти
...
# при наличен Postgres: пусни seed-а ДВА пъти → нула дубли в РЕАЛНАТА база
```
**`check-dups` не е „чисто = 0".** Той е текстов grep по `slug:` литерали и не различава `upsert` от
умишлен `update`-refresh — zabobovdol има базова линия от стотици такива (патърнът `seed-detailed-N.ts`).
Критерият е **нула НОВИ дублата спрямо броя преди промяната**, не абсолютното число.

Съдържанието е на **български**, проверено. Готово = нула нови дублата · integrity чист · `tsc` чист ·
фактите с източник или явно маркирани за човек.

**Кога да делегираш на Сийдъра:** нова тема, която иска проучване на факти (телефони, институции,
графици), или голям пакет от отворени данни — неговата памет пази проверените източници и базовата
линия на `check-dups`. За малка локална поправка на съществуващ seed този ред стига.
