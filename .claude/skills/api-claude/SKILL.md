...
name: api-claude
description: >-
  Reference for the Claude API / Anthropic SDK — model ids, pricing, params, streaming, tool use, MCP,
  agents, caching, token counting, model migration. Use whenever code calls Claude or Anthropic
  (anthropic and @anthropic-ai SDKs, claude-* model ids) or the task is LLM-shaped with the provider
  unstated (agents, tool definitions, RAG, LLM-judge, summarize, extract, classify); never answer
  model choice, pricing or limits from memory. Skip when another provider (OpenAI, Gemini, Mistral,
  Cohere, Ollama) is being worked on. Ползвай при код, който вика Claude или Anthropic API, избор на
  модел, цени, кеширане на промптове или агенти.
license: Complete terms in LICENSE.txt
source: anthropics/skills commit 33375500, Apache-2.0 (LICENSE.txt). Тялото е дословно; променено: преименувано от claude-api (резервиран префикс) и съкратено описание.
---
