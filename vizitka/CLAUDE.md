...
- **Портфейли (Apple Wallet + Google Wallet):** визитката се добавя в телефонния
  ...
  update web service e на `/v1/…` (токен `ApplePass`, HMAC върху slug); регистрациите
  на устройства са в таблица `apple_pass_registrations`. Смяна на текст на бутоните само
  през официалните артове при нужда — сега са локални SVG (`public/badge-*-wallet.svg`).
