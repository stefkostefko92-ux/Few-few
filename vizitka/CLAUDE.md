...
- **MCP конектор (`/mcp`)** — Vizitka е MCP сървър за ChatGPT и Claude. Ревизия **2026-07-28**
  ...
  Сверено със спецификацията (не по памет): `server/discover` е **MUST** (носи и
  инструкциите — модерната ера няма `initialize`); `server/discover` и `*/list` носят
  `ttlMs`+`cacheScope` (MUST); модерна заявка без хедър `MCP-Protocol-Version` или с
  хедър≠тяло → 400/-32020 (и между ерите); липсващ `clientCapabilities` → -32602 (не -32021);
  грешка на инструмент БЕЗ `structuredContent` (клиентите го валидират срещу `outputSchema`).
  Текстът на собственика в `fetch` е ограден като „данни, не инструкции“ (OWASP LLM01) и
  маркерите се махат от самия текст. `fetch` чете ЕДНА визитка с една заявка (без N+1);
  `search` е с таван 10 и при празна заявка. Съгласието носи дата (`ai_consent_at`, чл. 7(1)).
- **Статистика без бисквитка:** повторните преглеждания се отсяват с HMAC хеш (IP+браузър+
  визитка, сол сменяна при рестарт) САМО в паметта за 12 ч. — бисквитка за статистиката на
  собственика не е строго необходима (ePrivacy чл. 5(3)). Посетителите не получават бисквитки.
...
- **Портфейли (Apple Wallet + Google Wallet):** визитката се добавя в телефонния
  ...
  **никога** личната снимка (наш избор — минимизация; Google я позволява). В картата няма
  собствените бутони на потребителя (AUP на Google важи за всяка връзка), скритата визитка
  праща празен `EXPIRED` обект, а save линкът е „skinny“ JWT само с id-та — обектът се
  създава предварително (PATCH → 404 → POST), защото пълният JWT минаваше лимита от 1800
  знака. Бутонът за посетители е зад `GOOGLE_WALLET_PUBLISHED=1` (след одобрението);
  дотогава го виждат само собственикът и админът. Правата на service account-а се дават с покана „Developer“ в
