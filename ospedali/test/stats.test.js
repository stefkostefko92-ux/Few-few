...
//
// ВНИМАНИЕ: `percentile` има известен off-by-one (нестандартна семантика,
// индекс = floor((p/100)*n) вместо (p/100)*(n-1)) → напр. P90(1..10) дава 10, а
// не ~9.1. Това поведение е УМИШЛЕНО пиннато тук; поправката е отделна
// координирана стъпка след пуска и се прави отделно. НЕ „поправяй" теста.
...
test('percentile — ТЕКУЩА (нестандартна) семантика, замразена', () => {
  // Известен off-by-one: P90 на 1..10 връща 10 (индекс floor(0.9*10)=9),
  // не ~9.1. Пиннато нарочно — поправка отделно.
