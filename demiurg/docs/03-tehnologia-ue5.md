...
## 1. Версия — избрана **UE 5.8**

Решение на собственика: **UE 5.8** (най-новата линия). Долните функции са зрели още от 5.6 и
се подобряват през 5.7/5.8; проучването е правено спрямо 5.6, затова **сверявай версийно-
специфичните детайли (release notes на 5.8)** — принципите остават същите.

| Функция | Състояние | Релевантност за нас |
|---|---|---|
| **Nanite** | зрял; от 5.6+ Nanite Foliage, „Nanite Voxels“ (tech demo) | Само за **статичен** далечен детайл; **проблемен за динамична промяна** |
| **Lumen** | зрял; ускоряван HWRT през 5.6→5.8 | Динамично GI без bake — идеално за копаем/строим свят, но **скъп на GPU** |
| **World Partition** | production | **Задължителен** за голям стрийминг-свят |
| **PCG Framework** | production (значително ускорен от 5.6+), GPU path | Процедурно генериране на terrain/биоми |
...
**Прагматичен старт:** UE 5.8 + Voxel Plugin 2 + World Partition + Lumen (Nanite само за
