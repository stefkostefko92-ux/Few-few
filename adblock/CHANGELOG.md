...
Пълен одит на всичко досега: код (Кодаджията, Разбивача, Хромаджията) + визуален преглед
в истински Chromium (popup и настройки на 4 езика × 2 теми, landing и политика на десктоп
и мобилно, реални сайтове с и без разширението). Всичко по-долу е възпроизведено преди
поправката и покрито с тест.

**Счупени сайтове (съществуваше преди 5.0.5):**
- `antiadblock.css` слагаше на ВСЯКА страница `html, body { height: auto; position: static;
  overflow: auto }` — приложение на цял екран (`html, body { height: 100% }` с карта или
  редактор вътре) се свиваше до 0 px. Сега отключването важи само след като наистина сме
  махнали anti-adblock стена (`html.tbab-aab-unlocked`).
- Същият файл криеше всеки елемент, чийто клас СЪДЪРЖА „ad-block"/„adblock" — `head-block`,
  `thread-block`, `download-block`… изчезваха навсякъде. Сега само цели токени, които
  именуват стена (`adblock-modal`, `ad-block-notice`…), плюс `.fc-ab-root` (Google Funding
  Choices).

**Бисквитки:**
- След наш „Отхвърли" банер, който още е на екрана (анимация при затваряне, стъпка
  „потвърдете избора"), получаваше „Приеми" от следващия проход. Сега след отказ на
  страницата никога не приемаме; каквото не се затваря, се скрива след HARD_MS.
- „Pay or accept" стени: бутонът на iubenda при la Repubblica е «Rifiuta e abbonati» —
  отваря абонаментна стена, която покрива страницата. „Отказ", който води към плащане,
  вече не се брои за отказ; прескачаме го към безплатния изход («Continua senza
  accettare», добавен в CMP нивото) или, ако няма такъв, приемаме — както обещава
  описанието.
- Бутон с `position: fixed` никога не се натискаше (`offsetParent` е `null` за него).

**Регресии от 5.0.5 (наблюдателите само върху добавеното):**
- Правила, които хващат ПРЕДШЕСТВЕНИК на добавеното — `:has()` (~800 в EasyList), клас,
  сложен на съществуващ рекламен слот — вече пак работят (един `closest()` нагоре).
- Реклама/банер/стена, показани чрез смяна на атрибут (class/style/hidden/open) на вече
  съществуващ елемент, или стена, направена със смяна на текст — хващат се и след 40-ата
  секунда (5.0.4 ги хващаше само случайно, покрай друга промяна на страницата).
- Банер, добавен в fixed контейнер едно ниво по-дълбоко, пак буди прохода.
- Селектор, който парсерът „поправя" сам (`.a[title="x`), вече не поглъща всички
  селектори след него в „My filters"/абонаментите.
- Генераторът на генеричния CSS вече не приема висящ комбинатор (`div >`) и флаг `s` —
  иначе един такъв ред изпускаше чънк от 500 селектора.
- Стартовите/периодичните цени остават ниски: Speedtest-подобният тест дава ~930–1035
  Mbps срещу ~1000–1050 без разширение (5.0.4: ~630).

**Интерфейс:**
- Настройките бяха наполовина на английски на BG/IT/DE (етикетите на статистиката, Smart
  Detection, празните списъци, помощта за „Мои филтри", състоянието на ядрото, бутонът за
  дарение…) — 48 нови ключа на 4 езика. Единиците и времената са на езика на браузъра
  („4,6 GB", „55 мин", „преди 5 минути"), броят филтри е форматиран („43 709+").
- Долу в настройките пишеше „v5.0.0" — версията вече идва от manifest-а.
- Светла тема: картата на Carbon Stealth беше черен текст върху черно.
- „OK" в „Състояние на ядрото" вече не се пренася на два реда.
- Popup на BG: „Скрий рекламите тук" (старото се режеше с „…").
- Landing: хоризонтален скрол на мобилно (сиянието зад снимката на popup-а) — поправен;
  не твърди вече „v5.0.x в магазина" преди одобрение; `color-scheme: dark`.

**Пакет и CI:**
- `tools/package.sh` пакетираше и `_metadata/` — локалния DNR кеш на Chrome от „Load
  unpacked" (~4 MB, запазено име); сега е изключен и самопроверката пада, ако `_…` папка
  (освен `_locales`) влезе в zip-а.
- Браузърните тестове вече вървят и в CI (нов job `browser`, Playwright 1.56.1; без
  браузър падат, не се „пропускат").

## 5.0.5 — 2026-09-25

**Производителност: с включено разширение Speedtest показваше много по-ниска скорост (докладвано: ~900 → ~150 Mbps).**
Speedtest by Ookla мери през WebSocket-и, обработвани на главната нишка на страницата,
докато стрелката и числата се анимират — всяка милисекунда, която нашият код взима от
тази нишка, излиза от измерената скорост. Възпроизведено локално
(`tools/perf_speedtest.mjs`: голям DOM + анимация всеки кадър + 6 WebSocket-а): без
разширение ~1000 Mbps, 5.0.4 ~560–640 Mbps, 5.0.5 ~1000 Mbps. На живия speedtest.net
времето за стил за ~40 s пада от 1.3 s на 0.13 s. Три причини, всички поправени:

- **`cosmetic_generic.css` не можеше да се индексира.** 13 600 генерични селектора бяха
  в 28 правила `html[data-tbab-on] :is(<500 селектора>)` — Blink слага такова правило в
  „универсалната кофа", значи ВСЕКИ елемент при ВСЕКИ style recalc се сверяваше с
  всичките. Измерено: пълен restyle ~47× по-скъп от страница без него (първият стил на
  голяма страница 600 ms вместо 70 ms). Сега е едно вложено правило (CSS nesting) и всеки
  селектор се индексира по своя id/клас: ~1.1×. Същите 13 633 селектора (сверено в
  Chromium); в общите списъци влизат само селектори с доказана граматика, останалите са
  отделни правила — невалидният пада сам. Генераторът е `tools/generic_css.mjs`.
- **`cookies.js` правеше ~150 пълни заявки към документа на всеки 400 ms, докато
  страницата се променя, а в „дълбок" режим обхождаше целия DOM веднъж за ВСЕКИ
  селектор** (търсене на shadow root-и). Сега: shadow root-ите се събират веднъж на
  проход, всеки списък е една заявка, кликовите нива се пропускат, когато няма банер
  или CMP бутон, и проход се пуска само при промяна, която може да донесе банер
  (добавен елемент горе в `body`, диалог, известен CMP; атрибути на html/body).
- **`content.js` сканираше целия документ всеки кадър при всяка промяна** (вкл. само на
  текст — часовник, стрелка) и четеше layout за Smart Detection. Сега сканира само
  добавените поддървета с една заявка на списък; при промяна само на текст — нищо
  (освен ако сайтът има процедурни `:has-text()` правила); Smart Detection — най-много
  веднъж на 500 ms. `antiadblock.js` също вече не реагира на промени само на текст.

Тестове: `build_filters` — генераторът (индексируеми списъци, изолиране на невалидни
селектори, скоби в низове) и „без :is() мега-списък"; в истинския Chromium — генеричният
CSS пак крие, реклама и CMP банер, добавени късно и дълбоко в приложение, пак се хващат,
и пазач „restyle с генеричния CSS < 4× страница без него" (старата форма: ~47×).

## 5.0.4 — 2026-09-24

Пълен одит с шест агента (Кодаджията, Разбивача, Хромаджията, SEO, Правния, Скоростника)
+ собствен преглед. Всяка находка е проверена, поправена на верния слой и носи тест.

**Сигурност — спешно (засяга и живата 5.0.3 в стора):**
- **cookies.js кликваше по ВСЯКА страница.** Текстовото съвпадение („Accept“, „Decline“,
  „OK“) и генеричните `button[aria-label*=…]` търсеха в целия документ: приемаха LinkedIn
  покани, отказваха входящи повиквания, потвърждаваха „OK“ диалози, приемаха нови условия за
  ползване. Червеният екип показа и вектор за атака: враждебна страница НАВИГИРА потребителя
  до OAuth екран на друг сайт, където скриптът натиска „Allow“/„Accept“ (без iframe, значи
  X-Frame-Options не помага). Сега: CMP-специфичните бутони (OneTrust, Cookiebot, …) остават
  глобални; всичко генерично само вътре в контейнер, който реално говори за бисквитки; никога
  бутон, който изпраща форма (освен Google consent), никога линк извън страницата; нищо
  генерично на страници за вход, OAuth и плащане. `form[action*='consent'] button` и
  `aria-label*='allow'` са премахнати изцяло. Мутация срещу стария код: 30 клика на „Decline“
  за 8 s във фикстура с повикване.
- **`:remove-attr()` и `:style()` без политика** (червен екип): абонаментна листа или live
  каналът можеха да махнат `sandbox` от iframe (излизане от sandbox) или да сложат
  `url(…)` beacon. `scriptlets/policy.js`: `styleOk` (без url()/image-set()/attr()/escape-и),
  `removeAttrOk` (никога атрибут от ATTR_DENY, и regex, който може да го хване),
  `proceduralOk` в `safeSelector`; политиката вече се зарежда и в изолирания свят преди
  `content.js` и се прилага при изпълнение за всеки източник, включително „My filters“.
- **Service worker-ът вече различава content script от страница на разширението:**
  страница може да поиска само `smartHit`, `getCosmetic`, `saveCustomSelector`, `ytBypass`;
  toggle, allowlist, абонаменти, импорт, дневници на други табове — само от popup/options.
  `saveCustomSelector` записва под хоста на изпращача, не под `msg.host`, и минава политиката.
- **Anti-rollback само между подписани версии** + версията трябва да е безопасно цяло число:
  на браузър без Ed25519 MITM с `version: Number.MAX_VALUE` заключваше всички бъдещи ъпдейти.

**Коректност (Кодаджията):**
- `tools/build_filters.mjs`: едно извикване още сочеше преименуваното `priorityOf` →
  седмичният ребилд на листите гърмеше с ReferenceError при зелени тестове (те само грепваха
  кода). Нов тест пуска конвертора наистина върху фикстури (`--out` в temp). Той хвана и втори
  бъг: знаците за опции не включваха `_`/`[`/`]`, затова `$redirect=googletagmanager_gtm.js`
  ставаше мъртво правило с литерален `$` (сега истински redirect към сурогата).
- Пауза по време на ъпдейт на разширението оставяше защитата изключена до рестарт на браузъра.
- `enabled` вече не се синхронизира (паузата на устройство А изключваше Б без будилник);
  студено събуден service worker вече не изпуска sync промени.
- Кирилски домейн в allowlist-а (`пример.бг`) тихо проваляше ВСИЧКИ следващи allowlist
  промени (DNR приема само punycode, а обновяването е атомарно) — нормализация към punycode,
  валидация на хоста, невалидните записи не стигат до DNR; scriptlet-ите се изключват и на
  IDN/IP/localhost allowlist хостове.
- Allowlist-ът се зачита навсякъде: `cookies.js`, `antiadblock.js`, `meta.js` и промените в
  `content.js` (нови филтри, Smart Detection); `meta.js` зачита и „без козметика“.
- Вградените рекламни селектори съвпадат като начало на клас, не като подниз
  (`ad-container` криеше `thread-container`, `download-container`…), и спазват `$generichide`.
- Picker-ът не може да запише селектор за `<body>`/`<html>` (празен сайт при всяко зареждане).
- Архивът с настройки вече включва „My filters“.
- YouTube: наблюдателят действа само ако клипът изобщо не е тръгнал (не при буфериране по
  средата), не пипа живи излъчвания и стъпка 2 (6 ч bypass) идва само ако забиването се върне
  до 3 минути след стъпка 1.
- „Blocked on this page“ показваше „?“: Chrome не дава URL на блокираната заявка в
  пакетирано разширение. Сега е честна разбивка по филтър-листи (EasyPrivacy ×12, EasyList ×7…)
  на четирите езика.
- Деплой: `filters.json` се публикува само заедно с валидния си подпис (staging + mv); без
  ключ старата подписана двойка остава и деплоят сигнализира.

**Landing страница (пренаписана):** реалният popup вместо три вградени base64 копия
(137 KB → 55 KB HTML), „Nine reasons to switch“, честна сравнителна таблица с uBO Lite с дата
и източник (без колоната „Others“ и без невярните клетки), чеклист на функциите, поправен CSS
на „How it works“ (беше в `<noscript>`), FAQ = JSON-LD, импресум (Carbon Stealth VCC, ЕИК
208725180, адрес), бутон „Pause motion“ (WCAG 2.2.2), светкавици без `shadowBlur` и с цикъл
само докато има какво да се рисува и всяка мълния растеризирана веднъж (TBT на мобилно 1048 ms →
108 ms, десктоп 601 ms → 0 ms; лабораторно, 5 рана, 4× CPU), ≤2 проблясъка в секунда,
без разклащане на страницата, фон с compositor-only анимация, контрасти, фокус, таблица с
`th scope`. Точни твърдения за данните („browsing data never leaves your device“).

**Поверителност и сървър:** политиката е пренаписана по чл. 13 GDPR (администратор, правно
основание, получатели, трансфери, дарения през Revolut, права, КЗЛД); `PRIVACY.md` = сайта.
nginx: `access_log off` и само критични грешки; Caddy: без `log`; заглавки HSTS, nosniff,
Referrer-Policy, CSP `frame-ancestors 'none'`… — проверени на живо с nginx и Caddy.
`robots.txt` с една група (именуваните групи отменяха забраната за `filters.json`).

Тестове: `npm test` 9 файла (нови: `build_filters`, `cookies`; разширени: `background`,
`youtube`); `npm run test:browser` — 24 фикстури за бисквитки + 9 проверки в истинското
разширение в Chromium.
...
разширението се изключи — трета форма на отказ, различна от диалога и от грешката с
Playback ID (5.0.1), която нито един от двата детектора не виждаше.

- **Stall watchdog** (`youtube_skip.js`): поискано възпроизвеждане (`paused === false`),
  `readyState` под HAVE_FUTURE_DATA и нула напредък на `currentTime` за 25 s (не на пауза,
  не приключило, не офлайн, не по време на bypass, не по време на форсирана реклама) =
  забиване. Възстановяване на стъпала, най-евтиното първо, по един reload на стъпало на
  таб: **(1)** reload без request флаговете — единствената сървърно видима разлика между нас
  и обикновен клиент; ако това е било, блокирането (клиентско рязане) остава; **(2)** reload
  като чист клиент (bypass, същият ограничен път като enforcement: макс. 2 на 6 ч).
  `youtube_loader` подава `disableRequestFlags` на `youtube_main` при стъпка 1.
- Enforcement/bypass логиката е извадена в `tryBypass(now)` и се споделя от трите детектора.
- `tests/youtube.test.mjs`: 39 случая (+ stall стъпка 1/2, четири „не е stall" случая,
  loader без флагове).
- Диагностика за следващия доклад: при stall в конзолата на страницата остава
  `Supreme AdBlock: YouTube playback stalled …`; popup-ът показва „Blocked on this page".

## 5.0.1 — 2026-09-13

Поправка: YouTube клипове от време на време не зареждат (мъртъв плейър / грешка при
възпроизвеждане), без възстановяване. Средата за разработка не стига до YouTube, затова
диагнозата е по кода + публичните доклади за 2026 г. („playback error" срещу адблокъри),
а всяка поправка носи тест в `tests/youtube.test.mjs` (25 случая; регресионният за
bypass-а пада срещу стария код — проверено с мутация).

- **Първопричина (доказана в кода):** сесийният флаг `tbab_yt_bypass` беше голо „1" без
  срок и надживяваше 6-часовия bypass на service worker-а. Сценарий: enforcement →
  bypass + reload (чист клиент, реклами) → след 6ч bypass-ът изтича, но табът стои
  отворен: loader-ът вече е решил да не инжектира, а `youtube_skip` се събужда и пак
  форсира пропускане на рекламите → YouTube ни детектира отново → enforcement, но
  флагът казва „вече bypass-ваме" → **нула reload, диалогът скрит от youtube.css, мъртъв
  плейър без изход**. Сега: авторитетът е `ytBypassUntil` (storage), сесията пази само
  КОГА табът е reload-вал (loop guard 90s); enforcement при изтекъл bypass → нов bypass +
  reload; enforcement по време на bypass → една втора възможност на страница; при
  невъзможност да се въоръжи bypass диалогът на YouTube се показва вместо мъртъв плейър.
- **Козметиката на YouTube спира по време на bypass** (`content.js`): EasyList правилата
  за youtube.com (16 селектора за ad-slot контейнери) продължаваха да крият елементи в
  „чистия" клиент — детектируемо. Гейтват се като `youtube.css`.
- **Без seek към края на видеото** при `.ad-showing`: при server-side stitched реклами
  (SSAP) `duration` е на целия поток (реклама + клип) и seek-ът приключваше самия клип
  (черен плейър / autoplay към следващия). Остават ускорение ×16, mute и нативният Skip.
- **Резервен път за флага** (`youtube_main.js`): ако флагнатият `/player` отговор е
  UNPLAYABLE/ERROR/LOGIN_REQUIRED, една повторна заявка без флагове; ако тя е
  възпроизводима — флагът е причината: отговорът ѝ отива в плейъра (рекламните полета
  пак се режат клиентски) и флаговете спират за страницата. Недостъпно видео и в двата
  случая → оригиналният отговор, флаговете остават.
- **Грешка с Playback ID без диалог** (формата от 2026: „An error occurred… (Playback
  ID: …)") се третира като enforcement — само извън bypass; „Video unavailable" без
  Playback ID (изтрито/частно видео) не се пипа.

## 5.0.0 — 2026-09-09

### Фаза 6 — pre-flight с 4 агента (Хромаджията · Кодаджията · Тайният агент · Качествения)
- **Логото е обратно фирменият щит.** В 4.6.0 иконите бяха прерисувани „Cosmic
  Slate“ (тъмна плочка с мълния) — това замени щита на собственика. Сега
  `tools/generate_icons.py` не рисува нищо: чете бранд файловете
  (`store/brand/shield-transparent.png`, `logo-transparent.png`, мастер
  `server/icon-512.png`) и само ги смалява — PNG четец + Lanczos-3 върху
  премултиплицирана алфа, пак нула външни библиотеки, нула увеличаване.
  Прегенерирани: `icons/16-32-48-128`, store иконата, промо плочката (светъл фон
  — надписът в логото е тъмносин и се губеше върху карбон), marquee,
  скрийншотите. Сайтът: `favicon.svg` (последното останало „Cosmic Slate“) е
  премахнат — табът вече ползва `favicon-48.png`/`icon-512.png`, тоест същия щит
  като `og.png` и apple-touch иконата.
