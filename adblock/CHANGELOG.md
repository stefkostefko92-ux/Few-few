...
**Блокиране: Turtlecute 118 → 130 от 132** (две еднакви рунда, същите условия; adblock-tester
непроменен — 95), без да губим защитата срещу anti-adblock детектори.

- Причината за 6 от пропуските: два глобални uBO филтъра (`||doubleclick.net^$xhr,redirect=
  noop.txt`, `||pagead2.googlesyndication.com^$xhr,redirect=noop.js`) пренасочват ВСЯКА XHR
  заявка към тези рекламни мрежи в празен отговор — за да заблудят детекторите на блокери.
  Тестът (и всеки детектор) вижда „успех" → „не е блокирано". Детекторите обаче ползват за
  стръв ИСТИНСКИ рекламни ресурси (`adsbygoogle.js`, `gpt.js`, `/pagead/id`, `/gampad/…`),
  затова `build_filters.mjs` вече стеснява тези пренасочвания точно до тези пътища; всичко
  друго към рекламната мрежа се блокира. Специфичните за сайт пренасочвания не се пипат.
- Още 6 рекламни/телеметрични хоста само като трета страна (никога на собствения им сайт —
  youtube.com не е засегнат): `ads.youtube.com`, `gemini.yahoo.com`, `adtech.yahooinc.com`,
  `metrika.yandex.ru`, `ads-api.twitter.com`, `grs.hicloud.com`.
- Тестове: стесняването и новите хостове са гейтнати в `tests/rules.test.mjs`; browser 56/56.

## 5.1.1 — 2026-09-25

Одит на 5.1.0 с осем от нашите агенти (Хромаджията, Кодаджията, Разбивача, Изпитателя,
SEO, Правния, Тайния агент, Преводача). Всяка потвърдена находка е поправена и покрита
с тест; нищо не е прието на доверие — проверено в node и в истински Chromium.

**Сигурност:**
- Пауза/изключване вече спира ВСИЧКО: динамичните правила (live канал, „Моите филтри",
  листите от автора) продължаваха да блокират — не са рулсети. Едно allowAllRequests
  правило с най-висок приоритет, само докато е на пауза.
- Листите от автора (неподписани данни от трета страна) дават само block/allow: без
  redirect, заглавки, allowAllRequests и правила за самата страница.
- Селектор не може да излезе от CSS блока (`{`, `}`, `;`, коментар, завършваща `\`)
  нито да скрие цялата страница (`body.x`, `.a, body`, `body:has(…)`).
- Live scriptlet-и само от подписан filters.json (Chrome < 137 няма Ed25519 → само данни).
- Content script никога не говори за чужд хост (about:blank → origin на рамката).
- `!#include` към друг произход се пропуска при билда.
- Процедурни правила: пълен проход най-много на 250 ms, не на всеки кадър; regex върху
  ≤4000 знака.

**Коректност:**
- Отказ на Chrome за листите (квота) вече не оставя EasyList & co. изключени след пауза:
  втори опит само с основните рулсети; неприетият избор се връща.
- Листите от автора имат фиксиран слот; осиротели правила се махат.
- uBO `поддомейн#@#+js(…)` вече отменя директивата на родителския домейн.
- Статичният `content.css` (.ad-container и сродни) вече се подчинява на allowlist/пауза.
- `lang` на страниците — пълният таг (fil, zh-TW).

**Лицензи и твърдения:** пълните текстове в `licenses/`; THIRD_PARTY_NOTICES казва какво
сме променили и откъде е непромененият източник (URL, дата, SHA-256). 31 вградени
регионални листи (не 33). Политиката — „единствената заявка КЪМ НАС" + листите на GitHub
(САЩ). Сравнението с uBOL — външни листи от юни 2026. Landing: „Measured, not claimed" —
два публични теста при еднакви условия за 6 блокера, вкл. където Ghostery е пред нас.

**Landing — бурята:** мълнията вече е по рецептата на „Двубой в Рейвънхолд" (`boy/`):
WebGL 2 без библиотеки — стъпков лидер, после обратен удар като ДВА импулса (70 ms, втори
след 160 ms — никога стробоскоп), бурени облаци и дъжд, осветени отвътре от светкавицата,
искри с motion blur, float HDR буфер → bloom (5 нива) + анаморфна ивица → ACES → зърно.
Рендерира само докато трае удар; стартира след `load` (LCP е текстът); бавен GPU/загубен
контекст → досегашната Canvas 2D буря; reduced motion/пауза → нищо. ≤ 2 импулса в секунда,
осветената площ е ограничена в пиксели (WCAG 2.3.1).

**Магазини:** единствена цел „content blocker"; Edge — само `docs/listing` (без Chrome);
AMO — изходният код е задължителен → `dist/…-source.zip` (git archive);
`scriptlets/list_ubo.txt` (1 MB) извън пакета; `tools/build_store_kit.py` в репото.

**Преводи:** по-ясни `opt_downloaded_from` и `cosmeticsTitle` във всичките 73 локала,
точкови корекции (de, tr, bg, it), листингите — 31 листи + разкрита промо карта.

## 5.1.0 — 2026-09-25

Най-голямото разширение на покритието досега: повече листи, режим „Фокус", страница за
счупени сайтове, 70 езика и пакет за Firefox. Нищо ново не изпраща данни навън.

**Филтър-листи (`tools/lists.json` → `rules/list_*.json`, `rules/cosmetic_*.json`):**
- Конверторът е изваден в `lib/abp2dnr.js` — един и същ код в билда (vm) и в service
  worker-а (за листите, които се свалят от автора). Поддържа `$badfilter`, `$all`,
  `$to`/`$from`, `$denyallow`, `$method`, `$removeparam`, `!#include`, `!#if` (env_chromium,
  env_mv3, cap_user_stylesheet, ext_ublock), hosts формат и IDN → punycode.
- uBlock Origin filters (6 901 мрежови + 10 172 козметични) и Peter Lowe (3 465 домейна)
  — включени по подразбиране. Общо 60 000+ правила по подразбиране.
- 31 регионални листи с проверени лицензи; тази за езика на браузъра се включва сама.
- Български, полски (NC), скандинавски и хърватски нямат лиценз за разпространение — не
  са в пакета; при включване браузърът ги сваля от автора и ги конвертира локално като
  ДАННИ (динамични правила от id 200000, ≤5000 на листа). Българската е „препоръчана",
  не автоматична — включването ѝ значи заявка към трета страна.
- Невалиден `urlFilter` (`||*`, `|` по средата, двоен `||`) отхвърля ЦЕЛИЯ ruleset в
  Chrome — `normalizeUrlFilter` + `opt:unparsable` ги оправят или пропускат. `OPT_RE`
  вече приема `/` (uBO `redirect=google-analytics.com/ga.js` никога не се е парсвал).
- uBO scriptlet-ите — 64 парчета по хост (`scriptlets/ubo/cNN.js`), регистрирани само
  за своите хостове; нов routine `no-xhr-if` (общо 19). Данните се ескейпват, за да не
  съдържат `eval(`/`new Function`.
- Генеричният CSS на всяка листа се инжектира с `insertCSS` (`origin: "USER"`), гейтнат
  с `html[data-tbab-on]` — индексируем, без `:is()` списъци.
- 29 нови трекер крайни точки в `ad_rules` (id 238), намерени с `testMatchOutcome` срещу
  публичните тестове.
- `THIRD_PARTY_NOTICES.txt` — лицензите на всички листи, линк от Настройки.

**Режим „Фокус":** отделни превключватели за соц. уиджети, чат балони, бюлетини,
„разреши известия", AI изскачащи прозорци, други досадни, YouTube Shorts / Reels и
„Вход с Google" (One Tap). Всички изключени по подразбиране.

**„Сайтът е счупен?":** линк в popup-а → `report/report.html` — бързи поправки (без
скриване за сайта · разреши сайта · пауза 30 мин, после презарежда таба) и готов
`mailto:` доклад, който потребителят чете и праща сам. Пълният адрес — само с отметка.

**Бисквитки:** брояч на отказаните банери в popup-а (`cookieRejected` — нов тип в
`CONTENT_MESSAGES`, само брой, без адрес).

**Езици: 70** (73 папки в `_locales`, 196 ключа), вкл. RTL (ar, he, fa, ur) —
`@@bidi_dir`, логически CSS свойства, огледален превключвател. Листинг текст за всеки
език в `docs/listing/`. `tests/i18n.test.mjs` гейтва ≥54 езика, пълни ключове, същите
`$1` плейсхолдъри, `extDescription` ≤132. Преводите извън en/bg/it/de са машинно
подпомогнати — нужен е преглед от носител на езика.

**Firefox / Edge:** `tools/package.sh` прави и `dist/supreme-adblock-<v>-firefox.zip`
(event page, Gecko id, `data_collection_permissions`); `web-ext lint` — 0 грешки.
Разширението работи и без `getMatchedRules` (Firefox) — дневникът се скрива.

**Измерване:** `tools/compare_blockers.mjs` — същите публични тестове (Turtlecute,
adblock-tester) за нас и конкурентите, с дата и метод; числата на landing страницата
идват оттук. `tools/perf_speedtest.mjs --ext` — цена на главната нишка и за други блокери.

**Popup/Настройки:** карта „Филтър-листи" (имена през `Intl.DisplayNames`, значки „вашият
език" / „препоръчана" / „от автора", лиценз), карта „Фокус", `[hidden]` вече не се
отменя от `display:flex` (празна икона и линк се виждаха).
