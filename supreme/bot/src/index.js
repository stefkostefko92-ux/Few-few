...
app.post("/internal/ai-reply", async (req, res) => {
  const { channelId, content, ticketId } = req.body;
  ...
  try {
    ...
    await channel.send({
      embeds: [{
        ...
        title: "Automated AI Reply",
        description: content,
        color: 0x5865f2,
        fields: [
          {
            name: "⚠️ AI Disclosure",
            value: "This response was generated by an AI language model (Anthropic Claude). It may contain errors or inaccuracies. A human staff member will follow up shortly for confirmation.",
            inline: false,
          },
        ],
        footer: {
          text: "Supreme Bot · AI Auto-Reply · Powered by Claude",
        },
        timestamp: new Date().toISOString(),
      }],
    });
