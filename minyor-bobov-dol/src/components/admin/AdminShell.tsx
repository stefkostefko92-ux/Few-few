  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <header className="border-b border-slate-200 bg-white">
        <div className="flex h-14 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <Link href="/" target="_blank" className="text-brand-800 hover:underline">
              Виж сайта ↗
            </Link>
      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-4 py-6">
        <aside className={"w-60 shrink-0 lg:block " + (open ? "block" : "hidden")}>
          <nav className="space-y-1">
            {link("/admin", "Табло")}
            <div className="px-3 pb-1 pt-4 text-xs font-semibold uppercase text-slate-600">
              Съдържание
            </div>
            {RESOURCES.filter((r) => !r.adminOnly || user.role === "ADMIN").map((r) => (
