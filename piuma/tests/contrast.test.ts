...
/* Инвариант 12: „палитрата е мерена за контраст, не подбирана на око".
   Този тест е самото мерене. Чете реалните стойности от таблиците със стилове — нищо
   не се преписва тук, иначе тестът щеше да пази своето копие, не продукта.
...
// Токените и аврората живеят в `tokens.css`, компонентите (чиповете) — в `admin.css`,
// витрината — в `landing.css`. Мери се обединението: разделянето на файлове не бива да
// отваря дупка в гейта.
const CSS = ['public/tokens.css', 'public/admin.css', 'public/landing.css']
