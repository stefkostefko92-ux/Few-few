...
test('панел + агент: пълен път', { skip: !hasDatabase }, async (t) => {
  ...
  const keyId = keyHtml.match(/PIUMA_KEY_ID=([A-Za-z0-9]+)/)?.[1];
  const keySecret = keyHtml.match(/PIUMA_KEY_SECRET=(pk_[A-Za-z0-9_-]+)/)?.[1];
