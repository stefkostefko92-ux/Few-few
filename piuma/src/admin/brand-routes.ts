...
import { actorOf, optionalField, setFlash, stringField, tr } from './helpers.js';

const brandSchema = z.object({
  // Съобщението е КЛЮЧ — маршрутът го превежда на езика на потребителя при показване.
  slug: z.string().regex(/^[a-z0-9-]{2,40}$/, 'flash.slugRule'),
  name: z.string().min(2).max(120),
  summary: z.string().min(10).max(2000),
  voice: z.string().min(5).max(2000),
  language: z.string().regex(/^[a-z]{2}(-[A-Z]{2})?$/, 'flash.languageRule'),
...
brandRouter.post(
  ...
  async (req, res) => {
    ...
    const errors = [
      ...(input.success ? [] : input.error.issues.map((issue) => tr(res, issue.message))),
    ];
    ...
    setFlash(res, 'ok', tr(res, 'flash.brandCreated', { name: brand.name }));
...
brandRouter.get('/admin/brands/:id/edit', requireCapability('brands:manage'), async (req, res) => {
  ...
  res.render('admin/brand-form', {
    title: `${tr(res, 'common.brand')}: ${brand.name}`,
    brand,
...
brandRouter.post(
  ...
  async (req, res) => {
    ...
    setFlash(res, 'ok', tr(res, 'flash.brandUpdated'));
    res.redirect('/admin/brands');
  },
);
