...
postRouter.get('/admin/posts/new', requireCapability('posts:create'), async (_req, res) => {
  res.render('admin/post-form', {
    title: tr(res, 'form.composeTitle'),
    post: null,
    ...(await formContext()),
    errors: [],
  ...
  async (req, res) => {
    ...
    try {
      ...
      setFlash(
        ...
        findings.length
          ? tr(res, 'flash.draftSavedLint', { count: findings.length })
          : tr(res, 'flash.draftSaved'),
      );
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    if (!cfg.ANTHROPIC_API_KEY) {
      setFlash(res, 'error', tr(res, 'flash.generationOff'));
    ...
    try {
      ...
      setFlash(res, 'ok', tr(res, 'flash.draftsReady', { count: drafts.length }));
      res.redirect(`/admin/posts?status=DRAFT&brand=${encodeURIComponent(brand.slug)}`);
    } catch (error) {
      setFlash(
        res,
        'error',
        tr(res, 'flash.generationFailed', {
...
postRouter.get('/admin/posts/:id', requireCapability('posts:view'), async (req, res) => {
  ...
  res.render('admin/post-detail', {
    title: `${tr(res, 'common.publication')} · ${post.brand.name}`,
    post,
    ...(await formContext()),
  });
});
  ...
  async (req, res) => {
    ...
    try {
      ...
      setFlash(
        ...
        findings.length ? 'info' : 'ok',
        findings.length
          ? tr(res, 'flash.savedLint', { count: findings.length })
          : tr(res, 'flash.saved'),
      );
    } catch (error) {
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      setFlash(res, 'ok', tr(res, 'flash.postApproved'));
    } catch (error) {
      handleStateError(res, error, `/admin/posts/${id}`);
      return;
    }
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      setFlash(res, 'ok', tr(res, 'flash.postRejected'));
    } catch (error) {
      handleStateError(res, error, `/admin/posts/${id}`);
      return;
    }
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      if (Number.isNaN(when.getTime())) throw new PostStateError(tr(res, 'flash.invalidDate'));
      ...
      setFlash(res, 'ok', tr(res, 'flash.postScheduled', { date: res.locals.ui.dateTime(when) }));
    } catch (error) {
      handleStateError(res, error, `/admin/posts/${id}`);
      return;
    }
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      setFlash(res, 'ok', tr(res, 'flash.unscheduled'));
    } catch (error) {
      handleStateError(res, error, `/admin/posts/${id}`);
      return;
    }
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    if (!post || post.status !== 'APPROVED') {
      setFlash(res, 'error', tr(res, 'flash.onlyApproved', { status: post?.status ?? '—' }));
      res.redirect(`/admin/posts/${id}`);
      return;
    }
    await enqueuePublish(post.id);
    await audit(actorOf(req), { action: 'post.publish.now', targetType: 'Post', targetId: id });
    setFlash(res, 'ok', tr(res, 'flash.publishQueued'));
    res.redirect(`/admin/posts/${id}`);
  },
);
...
postRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      setFlash(res, 'ok', tr(res, 'flash.retryQueued'));
    } catch (error) {
      handleStateError(res, error, `/admin/posts/${id}`);
      return;
    }
