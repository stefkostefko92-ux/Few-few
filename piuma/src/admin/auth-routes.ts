...
authRouter.post('/admin/login', loginLimiter, async (req, res) => {
  ...
  const fail = (message: string): void => {
    res
      .status(401)
      .render('admin/login', { title: tr(res, 'auth.loginTitle'), error: message, next: '' });
  };
  if (!input.success) {
    fail(tr(res, 'flash.invalidLogin'));
    return;
  }
  ...
  if (!user || !user.active) {
    ...
    fail(tr(res, 'flash.wrongCredentials'));
    return;
  }
...
authRouter.get('/admin/2fa', (req, res) => {
  ...
  res.render('admin/2fa', { title: tr(res, 'auth.twoFactor'), error: null });
});
...
authRouter.post('/admin/2fa', totpLimiter, async (req, res) => {
  ...
  if (!verifyTotp(secret, stringField(req.body, 'code'))) {
    ...
    res.status(401).render('admin/2fa', {
      title: tr(res, 'auth.twoFactor'),
      error: tr(res, 'flash.wrongCode'),
    });
    return;
  }
...
authRouter.get('/admin/profile', requireLogin, async (req, res) => {
  const principal = humanPrincipal(req)!;
  const user = await prisma.user.findUniqueOrThrow({ where: { id: principal.user.id } });
  res.render('admin/profile', { title: tr(res, 'profile.title'), user, setup: null });
});
...
authRouter.post('/admin/profile/2fa/start', requireLogin, requireCsrf, async (req, res) => {
  ...
  if (user.totpEnabledAt) {
    setFlash(res, 'info', tr(res, 'flash.twoFactorOn'));
  ...
  res.render('admin/profile', {
    title: tr(res, 'profile.title'),
    user,
    setup: { secret, qrDataUrl },
  });
});
...
authRouter.post(
  ...
  async (req, res) => {
    ...
    const secret = decryptSecret(user.totpSecretEnc, config().TOKEN_ENC_KEY);
    if (!verifyTotp(secret, stringField(req.body, 'code'))) {
      setFlash(res, 'error', tr(res, 'flash.codeMismatch'));
      res.redirect('/admin/profile');
      return;
    }
    ...
    setFlash(res, 'ok', tr(res, 'flash.twoFactorSet'));
    res.redirect('/admin/profile');
  },
);
...
authRouter.post('/admin/profile/2fa/disable', requireLogin, requireCsrf, async (req, res) => {
  ...
  if (!(await verifyPassword(password, user.passwordHash))) {
    setFlash(res, 'error', tr(res, 'flash.wrongPassword2fa'));
    res.redirect('/admin/profile');
    return;
  }
  await prisma.user.update({
  ...
  setFlash(res, 'ok', tr(res, 'flash.twoFactorOff'));
  res.redirect('/admin/profile');
});
...
authRouter.post('/admin/profile/password', requireLogin, requireCsrf, async (req, res) => {
  ...
  if (!(await verifyPassword(current, user.passwordHash))) {
    setFlash(res, 'error', tr(res, 'flash.wrongPassword'));
    res.redirect('/admin/profile');
    return;
  }
  ...
  setFlash(res, 'ok', tr(res, 'flash.passwordChanged'));
  res.redirect('/admin/profile');
});
