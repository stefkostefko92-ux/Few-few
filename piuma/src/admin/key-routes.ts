...
import { actorOf, humanIdOf, setFlash, stringField, tr } from './helpers.js';
...
keyRouter.post('/admin/keys/:id/revoke', requireCsrf, async (req, res) => {
  ...
  if (!key || key.revokedAt) {
    setFlash(res, 'error', tr(res, 'flash.keyMissing'));
    res.redirect('/admin/keys');
    return;
  }
  ...
  setFlash(res, 'ok', tr(res, 'flash.keyRevoked', { name: key.name }));
  res.redirect('/admin/keys');
});
