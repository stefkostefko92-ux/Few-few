...
export function sparkline(points: SparkPoint[], options: SparklineOptions = {}): Sparkline {
  ...
  // Градиентът е за-заявка на всяка линия — идентификаторът трябва да е уникален на страницата,
  // иначе втората карта наследява запълването на първата.
  ...
  const svg = [
    ...
    '<stop class="spark-stop-top" offset="0"/><stop class="spark-stop-bottom" offset="1"/>',
    '</linearGradient></defs>',
    `<path class="spark-area" fill="url(#${gradientId})" d="${area}"/>`,
