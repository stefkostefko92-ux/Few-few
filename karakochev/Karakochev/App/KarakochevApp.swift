...
/// „Каракочев“ — записки с напомняне. Всичко живее на устройството:
/// няма акаунт, няма сървър, няма мрежа.
@main
struct KarakochevApp: App {
    ...
    private static func makeContainer() -> (ModelContainer, Bool) {
        ...
        do {
            ...
            print("[Каракочев] базата не се отвори (\(error.localizedDescription)) — временна база в паметта")
            do {
                let container = try ModelContainer(
                    for: schema,
                ...
                fatalError("[Каракочев] базата не може да се създаде: \(error)")
            }
        }
    }
}
