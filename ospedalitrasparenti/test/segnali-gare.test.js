...
test('clusterFrazionamento: целият прозорец се брои (4-то възлагане НЕ се изпуска)', () => {
  // 4 преки по 20k в рамките на 12 дни (window 30). Клъстерът обхваща ВСИЧКИ 4
  // (реалното раздробяване = 80k). Бивш дефект: кодът затваряше на 3-тото → 60k;
  // поправено — прозорецът се разширява до всички в рамките на `window`.
  const d = (day, importo) => ({ t: Date.parse(`2024-01-${String(day).padStart(2, '0')}`), importo });
  const r = clusterFrazionamento([d(1, 20_000), d(5, 20_000), d(9, 20_000), d(13, 20_000)], { window: 30, soglia: 40_000 });
  assert.equal(r.cluster, 1);
  assert.equal(r.valore, 80_000); // всичките 4 възлагания са в клъстера
});
