...
if [ -n "$HAS_DB" ]; then
  ...
  if $DC exec -T db pg_dump -U fivem fivem | gzip > "$BK"; then
    ...
    RAW="$(gzip -dc "$BK" | wc -c || echo 0)"
    if [ "${RAW:-0}" -lt 2000 ] || ! gzip -dc "$BK" | grep -q 'CREATE TABLE'; then
      rm -f "$BK"
      die "Бекъпът е празен или без схема (${RAW:-0} б. разархивирани) — спирам ПРЕДИ миграцията."
    fi
    ok "Бекъп: $BK ($(du -h "$BK" | cut -f1), ${RAW} б. разархивирани)"
  else
    rm -f "$BK"
