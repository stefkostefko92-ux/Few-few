...
## Проверено преди деплоя (25.09.2026, локален `docker compose` от този клон)

- **Билд.** И трите образа (backend, bot, frontend) се билдват.
- **Миграции.** Празна база → `migrate deploy` прилага и 39-те миграции, вкл. v50.
  Проверката на схемата минава; backend, frontend, Postgres и Redis са `healthy`.
- **`deploy/smoke.sh`.** 10 от 12 проверки са зелени срещу истинския nginx образ, вкл.
  проверките по съдържание (canonical, `image/jpeg`, robots/sitemap/llms). Двете червени са
  само заради бота, пуснат с фалшив токен — на сървъра ги покрива истинският `BOT_TOKEN`.
- **E2E на играта** срещу живия backend и Postgres:
  - `/daily`, магазинът (последна бройка) и улавянето при двоен клик минават точно веднъж;
  - 20 паралелни XP партиди за нов играч дават точен брой;
  - покупката пази снимка на ролята; изтрит артикул не трие покупката; изтичането маха ролята;
  - чужд сървър не може да лови; GDPR изтриването не оставя игрови редове.
- **Браузър през истинския nginx с неговия CSP.** 10 страници — нула CSP нарушения и нула
  JS грешки.
- **Откат към 3.4.0 срещу базата на v50.**
  - Без `SKIP_SCHEMA_CHECK=1`: backend-ът спира (exit 1).
  - Със `SKIP_SCHEMA_CHECK=1`: тръгва `healthy`. §5 е проверен на практика.
