...
import { latestEndedSeason, listSeasons } from "./seasons.js";
...
export async function closeSeasonIfEnded(now = new Date(), season = null) {
  ...
  const servers = (await prisma.gameSettings.findMany({ where: { enabled: true, ...notYet }, select: { serverId: true, announceChannelId: true, lastSeasonId: true }, take: 500 })) || [];
  // МОНОТОННО (одит на Разбивача 25.09.2026): сървър, затворил сезон, който
  // свършва СЛЕД target (или по същото време), не се връща назад. Преди удължен
  // S2 правеше „последния приключил“ отново S1 и сезонното XP се триеше наново.
  const endsOf = new Map(((await listSeasons()) || []).map((r) => [r.code, new Date(r.endsAt).getTime()]));
  const targetEnd = new Date(target.endsAt).getTime();
  let closed = 0;
  for (const s of servers) {
    const lastEnd = s.lastSeasonId ? endsOf.get(s.lastSeasonId) : undefined;
    if (lastEnd !== undefined && lastEnd >= targetEnd) continue;
