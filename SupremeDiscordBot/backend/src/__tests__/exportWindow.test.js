...
describe("двете страни ползват ЕДИН източник", () => {
  ...
  it("метлата за архиви ползва СЪЩИЯ прозорец (връзката съществува)", () => {
    // ВНИМАНИЕ — тук нарочно НЕ се твърди, че метлата се държи правилно.
    //
    // Първата версия проверяваше `toMatch(/continue/)` върху изрязано тяло на
    // задачата и беше доказано геймваема (Изпитателят, кръг 2): оставяш
    // `inExportWindow(` и някакъв `continue` в среза, чупиш поведението, тестът
    // остава зелен. Regex по сорса не може да съди поведение.
    //
    // Поведението вече се проверява с ИЗПЪЛНЕНИЕ на задачата —
    // `archiveCleanupBehaviour.test.js`. Тук остава само едното, което сорсът
    // наистина доказва: че метлата изобщо е вързана за общия прозорец, а не си
    // има собствено разбиране за срока.
    const code = read("services/scheduler.js").split("\n")
      .filter((l) => !l.trim().startsWith("//") && !l.trim().startsWith("*")).join("\n");
    expect(code, "метлата вече не ползва общия прозорец").toContain("inExportWindow(");
  });
