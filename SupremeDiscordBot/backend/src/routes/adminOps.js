...
router.post("/game/season", requireMainOwner, stepUp, async (req, res, next) => {
  ...
  try {
    ...
    if (!out.ok) return res.status(out.code === "DUPLICATE" ? 409 : 400).json({ error: out.error, code: out.code });
...
router.put("/game/season/:code", requireMainOwner, stepUp, async (req, res, next) => {
  ...
  try {
    ...
    if (!out.ok) return res.status(out.code === "NOT_FOUND" ? 404 : 400).json({ error: out.error, code: out.code });
