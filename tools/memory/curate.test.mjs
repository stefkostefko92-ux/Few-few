...
const judgementFixture =
  ...
  // Просрочена ПО КЛАСА на memory-freshness (наш код = 90 дни; котва към наш файл, без външен URL),
  // стара → пълният ход я вика „просрочено“. До 2026-09-09 тук стоеше „време-чувствителна >45 дни" —
  // отделна евристика, махната в полза на една дефиниция (тази на гейта).
  '- **2026-01-02:** Текущата версия на tools/agents/gate.mjs е v3.4 и това е stable release. _(обхват; high; "tools/agents/gate.mjs")_\n' +
  "\n## Карантина (непроверени — НЕ са факт)\n\n";
...
test("пълният ход ДАВА преценъчните сигнали (предпоставка — иначе следващият тест е празен)", () => {
  ...
  assert.match(r.out, /просрочено/, "просрочена по класа поука (наш код, 90 дни) трябва да се флагне");
});
...
test("--check НЕ произвежда преценъчните сигнали (парафраз/застаряло) — само механичния дедуп", () => {
  ...
  assert.doesNotMatch(r.out, /просрочено/, "--check пак пуска проверката за просрочване");
