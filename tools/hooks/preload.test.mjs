...
// Преди: „най-новите са последните във файла". Грешно — куката вмъква ОТГОРЕ, а курацията пренарежда
// (при Правния най-новата поука беше на позиция 241 от 254). Датата се чете от самия ред.
test("selectLessons: без задача → най-новите ПО ДАТА първо, независимо от позицията във файла", () => {
  const all = ["- **2026-07-01:** стара поука", "- **2026-09-20:** най-нова поука", "- **2026-08-01:** средна поука"];
  const r = selectLessons(all, "", 9999);
  assert.match(r[0], /най-нова/, "най-новата по дата е първа");
  assert.match(r[2], /стара/);
});
