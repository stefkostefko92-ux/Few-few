...
## Дневник на грешките и тренд (измерването, не гейтът)
- `errors.jsonl` — **всеки реален дефект** влиза тук и носи регресия. Регресията е `spec` (за
  поведенчески дефект на агент) **или** `test` (за дефект в нашите инструменти/куки) — двете са
  равностойни. Дотогава инструментът приемаше само `spec`, затова цял клас дефекти (байпаси в
  guard куки, тихи провали) **не можеше да бъде записан** и дневникът стоеше с 1 запис при
  десетина реални. Записвай с `node tools/agents/error-ledger.mjs add --agent <id> --desc "…"
  --test <път>`.
- `trend.jsonl` — **проследен в git** (беше в `.gitignore` → трендът умираше при нов клон/сесия).
  Пълни се от `eval.mjs --run <dir> --record`.
- `node tools/agents/defect-rate.mjs` — термометърът: колко нови дефекта откриваме, пада ли числото
  и **расте ли натискът** (spec-ове + тестове). Падащ дефектен процент при падащ натиск е сляпо
  петно, не зрялост — затова двете се четат само заедно. `--check` гейтва липсата на измерване
  (запис без регресия, тренд в `.gitignore`), не броя дефекти — да намериш дефект е добро.
