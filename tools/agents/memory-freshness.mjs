...
// НАШИЯТ КОД е най-бързо гниещият клас — измерено, не предположено (пресверка на карантината,
// 2026-08-04): 11 от 13 опровергани поуки на Наблюдателя и 12 от 27 на SEO описваха дефект, който
// ОТТОГАВА е поправен, при това поправката често носи коментар, преразказващ самата поука. Кодът е
// усвоил урока, паметта е останала със старото състояние.
//
// Тази проверка НЕ се покрива от `deep-audit` (dead-mem-path): там пътят е МЪРТЪВ; тук пътят е ЖИВ,
// редът съществува, но твърдението за него вече не е вярно. Мъртва котва се вижда от компютър;
// сменено съдържание — не.
//
// Долният коментар (запазен като история) отхвърляше срока с довода „ще произведе опашка от стотици
// просрочени". Измерих го: 427 проверени поуки цитират наш файл, но 402 от тях са на възраст под
// 30 дни, тоест при 90-дневен срок опашката ДНЕС е 25 записа — работеща, не стотици. Освен това
// `--check` гейтва ИЗМЕРВАНЕТО, не броя просрочени, значи класът не може да направи гейта червен;
...
// методът не остарява. Да ѝ сложа срок значи да произведа опашка, която никой не може да пресвери
// срещу нищо — фалшив показател, който обезсмисля механизма.
...
export function classify(line) {
  // Външният домейн бие: поука за Stripe, цитираща и наш файл, се съди по Stripe (по-волатилното
  // твърдение е за платформата). Репо-котвата хваща само поуките БЕЗ външен източник — точно
  // класът „състояние на нашия код", който гниеше без никакъв срок.
  for (const c of CLASSES) if (c.re.test(line)) return c;
  if (/https?:\/\//.test(line)) return DEFAULT_CLASS;
  return OUR_CODE.re.test(line) ? OUR_CODE : NO_EXPIRY;
}
...
async function main() {
  ...
  for (const [c, n] of Object.entries(sum.byClass).sort((a, b) => b[1] - a[1])) {
    // Легендата ЧЕТЕШЕ само CLASSES и затова показваше 365 за „наш код" вместо реалните 90 —
    // числото в отчета не отговаряше на числото, по което се смята срокът. Табло, което лъже за
    // собствения си праг, е по-лошо от липсващо табло.
    const days = [...CLASSES, OUR_CODE].find((x) => x.id === c)?.days;
    const label = c === NO_EXPIRY.id ? "не изтича (чиста методология)" : c === "изричен" ? "по поуката" : (days || DEFAULT_CLASS.days) + " дни";
