...
4. **Статичният префикс** (доктрина+процедура+споделено, ~5,15k т) е евтин за отделно пускане, но се
   ...
   **не вдигай тавана**. Старото твърдение „префиксът е ~40% от студена вълна" беше модел без измерване:
   `token-budget`/`flow-cost` описват СТАТИЧНИЯ дял, `usage-report` е истината за цената.
