...
- **Instagram Platform API** — 100 API-публикации/акаунт за плаващи 24ч (карусел = 1); нашият път е `publikator/` (чернова → човешко одобрение → публикуване), агентът стига до него през `tools/social/publikator.mjs` (само чернови, HMAC-подписано). Управлявана страница = план в панела + автопилот (седмично допълва чернови от медийната библиотека) + Insights (`publikator.mjs insights --brand slug`: views/reach/interactions по пост, дневен тренд на акаунта) — числата влизат в промпта на всяка чернова.
