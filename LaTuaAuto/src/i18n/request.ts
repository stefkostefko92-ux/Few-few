...
// Непълните преводи падат към it — нов език може да тръгне частично.
export default getRequestConfig(async ({ requestLocale }) => {
  const requested = await requestLocale;
  const locale = requested && isLocale(requested) ? requested : routing.defaultLocale;

  const fallback = (await import(`../../messages/${DEFAULT_LOCALE}.json`)).default as AbstractIntlMessages;
  const messages =
...
function deepMerge(base: AbstractIntlMessages, override: AbstractIntlMessages): AbstractIntlMessages {
  ...
  for (const [key, value] of Object.entries(override)) {
    ...
    // Масивите (faq.items, steps…) се ЗАМЕНЯТ, не се сливат — иначе стават обект.
    if (
