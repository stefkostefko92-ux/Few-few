  ...
  return (
    <>
      ...
      >
        ...
        <div>{uploading ? "Caricamento in corso…" : "Trascina qui le immagini o clicca per caricare"}</div>
        <small>JPG, PNG, WebP, GIF, SVG — max 12 MB</small>
      </div>

      {loading ? (
        <p style={{ color: "var(--ad-muted)" }}>Caricamento…</p>
      ) : media.length === 0 ? (
        <div className="ad-empty">Nessuna immagine caricata.</div>
      ) : (
            ...
            <div className="ad-media" key={m.id}>
              ...
              <div className="ad-media__body">
                ...
                <div className="ad-media__row">
                  <button className="ad-btn ad-btn--ghost" style={{ padding: ".35rem .6rem", fontSize: ".8rem" }} onClick={() => copy(m.url)}>Copia URL</button>
                  <button className="ad-btn ad-btn--danger" style={{ padding: ".35rem .6rem", fontSize: ".8rem" }} onClick={() => remove(m.id)}>Elimina</button>
                </div>
              </div>
            </div>
          ))}
